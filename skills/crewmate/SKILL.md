...
Implement only the supplied objective in the exact allowed paths. Apply the
packet's Ponytail mode to coding choices only: prefer the smallest change that
satisfies the approved contract. Do not redesign settled architecture or expand
authority. Stop and report a genuine design amendment when implementation
cannot satisfy the contract honestly.
