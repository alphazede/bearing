...
function hasUnsafeRecordPrototype(value) {
    if (!isObject(value))
        return false;
...
function hasUnsafeArrayPrototype(value) {
    return Array.isArray(value) && Object.getPrototypeOf(value) !== Array.prototype;
}
function hasPrototypePollution(value) {
    if (!isObject(value))
        return false;
    if (hasUnsafeRecordPrototype(value))
        return true;
    if (Object.hasOwn(value, "scope") && hasUnsafeRecordPrototype(value.scope))
        return true;
    for (const key of ["scores", "deficiencies"]) {
        const entries = value[key];
        if (hasUnsafeArrayPrototype(entries))
            return true;
        if (Array.isArray(entries) && entries.some((entry) => hasUnsafeRecordPrototype(entry)))
            return true;
    }
    return false;
}
function hasDenseOwnItems(value) {
    if (value.length > MAX_ITEMS || Object.keys(value).length !== value.length)
        return false;
...
function nestedUnexpectedKey(value) {
    if (isObject(value.scope) && hasUnexpectedKeys(value.scope, SCOPE_KEYS))
        return true;
    if (Array.isArray(value.scores)) {
        if (Object.keys(value.scores).length !== value.scores.length)
            return true;
        if (value.scores.some((score) => isObject(score) && hasUnexpectedKeys(score, SCORE_KEYS)))
            return true;
    }
    if (Array.isArray(value.deficiencies)) {
        if (Object.keys(value.deficiencies).length !== value.deficiencies.length)
            return true;
        if (value.deficiencies.some((item) => isObject(item) && hasUnexpectedKeys(item, DEFICIENCY_KEYS)))
            return true;
    }
    return false;
}
export function graderVerdict(report) {
    let weightedScore = 0;
    let totalWeight = 0;
...
export function parseGraderReport(value, approvedContractHash, approvedScope) {
...
export function selectGraderRoute(input) {
    const differentFamilyRoute = input.availableRoutes.find(({ provider }) => provider !== input.implementerProvider);
