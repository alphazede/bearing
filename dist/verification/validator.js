export const INHERITED_REASON_VERDICTS = {
...
export function composeValidatorVerdict(reasons) {
    if (reasons.some((reason) => REASON_VERDICTS[reason] === "FAIL"))
        return "FAIL";
    if (reasons.some((reason) => REASON_VERDICTS[reason] === "NEEDS_MORE_EVIDENCE"))
        return "NEEDS_MORE_EVIDENCE";
    return "PASS";
}
export function deriveValidatorEscalation(verdict, reasons) {
    if (verdict === "PASS")
        return "none";
    if (verdict === "NEEDS_MORE_EVIDENCE")
        return "re_execute_slice";
...
export function validateScope(scope) {
    ...
    if (scope.slices.length === 0)
        addReason("slice_unvalidated");
    for (const slice of scope.slices) {
        if (!slice.completion)
            addReason("slice_unvalidated");
        else if (!slice.completion.ok)
            addReason(slice.completion.reason);
    }
    const provenRequirements = new Set(scope.slices
    ...
    const passedCommands = new Set(scope.slices.flatMap((slice) => slice.evidence.filter((item) => item.status === "passed").map((item) => item.commandId)));
    const scopeHasUncoveredCommand = scope.slices.some((slice) => slice.evidenceCommandIds.some((id) => !passedCommands.has(id)));
    const sliceHasUncoveredCommand = scope.slices.some((slice) => {
        const slicePassedCommands = new Set(slice.evidence.filter((item) => item.status === "passed").map((item) => item.commandId));
    ...
    const completedSliceIds = new Set(scope.slices.filter((slice) => slice.completion?.ok === true).map((slice) => slice.sliceId));
    if (scope.readinessClaims.some((claim) => claim.sliceIds.length === 0 || claim.sliceIds.some((id) => !completedSliceIds.has(id)))) {
