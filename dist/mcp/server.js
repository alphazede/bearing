...
import { admitRepositoryRoot, applyLegacyExecutionContractApproval, applyLegacyRoleRouteApproval, executeHeadlessJourney, readDurableContinuation, readReviewContext, recordReviewGate, } from "../server/local-session.js";
...
const SERVER_INFO = { name: "bearing", version: "0.1.6" };
...
async function lockPath(repositoryPath, runId) {
...
function schemaViolation(rule, value, label = "arguments") {
    ...
    if (rule.type === "array") {
        ...
        if (typeof rule.minItems === "number" && value.length < rule.minItems)
            return `${label} has too few items`;
        if (typeof rule.maxItems === "number" && value.length > rule.maxItems)
            return `${label} has too many items`;
    ...
    if (rule.type === "integer") {
        if (!Number.isSafeInteger(value))
            return `${label} must be a safe integer`;
        if (typeof rule.minimum === "number" && value < rule.minimum)
            return `${label} is below its minimum`;
        return undefined;
    }
    ...
    if (typeof rule.pattern === "string" && !new RegExp(rule.pattern).test(value))
        return `${label} does not match its pattern`;
    return undefined;
}
...
function repositoryIdentity(repositoryPath) {
    return createHash("sha256").update(repositoryPath).digest("hex").slice(0, 16);
}
function toolResult(body, failed, modern) {
    return {
...
function rejection(runId, code, extra = {}) {
    return {
...
function continuationBody(read) {
...
async function continuation(repository, runId) {
    const read = await readDurableContinuation(repository, runId);
    if (read.ok)
        return continuationBody(read);
 ...
 */
async function withRunLock(repositoryPath, runId, operation) {
    let pinned;
    try {
...
function guard(read, args) {
    if (!read.ok && read.code !== "run_not_found")
        return rejection(args.runId, read.code);
...
async function transition(args, deps, sessionId) {
    try {
        ...
        if (args.action === "status")
            return continuation(args.repository, args.runId);
        const repositoryPath = read.repositoryPath;
        if (repositoryPath === undefined)
            return rejection(args.runId, "repository_rejected");
        ...
        if (!route)
            return rejection(args.runId, "route_unspecified");
        const locked = await withRunLock(repositoryPath, args.runId, async () => {
            ...
            const run = deps.headlessJourney ?? ((request) => executeHeadlessJourney(request, { processRunner: new NodeProcessRunner() }));
        ...
        if (!locked.ok)
            return rejection(args.runId, "run_busy");
        const { blocked: lost, receipt } = locked.value;
        if (lost || !receipt)
            return lost ?? rejection(args.runId, "transition_unavailable");
...
async function focusBegin(repository, requestPath) {
    try {
        const repositoryPath = await admitRepositoryRoot(repository);
        if (repositoryPath === undefined)
            return refusal("repository_rejected");
...
async function focusValidate(repository, focusRunId, receiptPath) {
    try {
        const repositoryPath = await admitRepositoryRoot(repository);
        if (repositoryPath === undefined)
            return refusal("repository_rejected");
...
async function reviewContext(args) {
    try {
        ...
        if (!read.ok)
            return refusal(read.code, { runId: args.runId });
 ...
 */
async function bindLegacyRoleRoutes(args, ownerSessionId) {
    try {
        const repositoryPath = await admitRepositoryRoot(args.repository);
        if (repositoryPath === undefined)
            return refusal("repository_rejected", { runId: args.runId });
...
function toolBody(name, input, deps, sessionId) {
    switch (name) {
        case "bearing_attach":
        case "bearing_handoff":
            return continuation(input.repository, input.runId);
        case "bearing_transition":
            return transition(input, deps, sessionId);
        case "bearing_focus_begin":
            return focusBegin(input.repository, input.requestPath);
        case "bearing_focus_validate":
            return focusValidate(input.repository, input.focusRunId, input.receiptPath);
        case "bearing_review_context":
            return reviewContext(input);
        case "bearing_review_record":
            return reviewRecord(input, sessionId);
        case "bearing_bind_legacy_role_routes":
            return bindLegacyRoleRoutes(input, sessionId);
        case "bearing_bind_legacy_execution_contract":
            return bindLegacyExecutionContract(input, sessionId);
    }
}
async function bindLegacyExecutionContract(args, ownerSessionId) {
    try {
        const repositoryPath = await admitRepositoryRoot(args.repository);
        if (repositoryPath === undefined)
            return refusal("repository_rejected", { runId: args.runId });
...
export function createDispatcher(deps = {}) {
    ...
    return async (request) => {
        ...
        if (version !== undefined && !MCP_PROTOCOL_VERSIONS.includes(version)) {
        ...
        const respond = (result) => (notification ? null : reply(id, result));
        const fail = (code, message, data) => (notification ? null : rpcError(id, code, message, data));
        switch (request.method) {
            ...
            case "tools/call": {
                ...
                if (violation !== undefined)
                    return fail(-32602, "Invalid tool arguments", { violation });
                const body = await toolBody(tool.name, args, deps, sessionId);
...
export function serveStdio(handle, input = process.stdin, output = process.stdout) {
    return new Promise((finish) => {
        ...
        const enqueue = (task) => {
            queue = queue.then(task).then((response) => { if (response)
                output.write(`${JSON.stringify(response)}\n`); }, () => undefined);
        };
        const oversized = () => Promise.resolve(rpcError(null, -32600, "Request line exceeds the stdio bound"));
        const line = (value) => {
            const trimmed = value.trim();
            if (trimmed.length === 0)
                return;
            if (trimmed.length > MAX_STDIO_LINE) {
                enqueue(oversized);
                return;
            }
            let parsed;
