...
import { BearingStore, BearingStoreError, isCompactedRunState } from "../store/bearing-store.js";
import { AdapterVerification, ReadinessService, REASONING_LEVELS } from "../onboarding/readiness.js";
...
import { derivePlanningState, next, planningValidationSignal } from "../journey/planning-state.js";
...
import { canonicalizeFitOwnerAnswer, FIT_OWNER_ANSWER_REMEDY, isFitDiagnostic } from "../journey/repository-fit.js";
...
export function measureImprovementWindow(window) {
    const coordination = [];
    const sliceAttempts = [];
    const grading = [];
    ...
    const tokenReports = [];
    const confirmedFindingSlices = new Set();
    ...
    for (const record of window.records) {
        ...
        if (typeof sliceRef !== "string" || sliceRef.length === 0)
            continue;
        ...
        if (record.signal === "grader_score") {
            grading.push({
                ...
                ...(confirmedFindingSlices.has(sliceRef) ? { groundTruth: "fail" } : {}),
            });
        }
    }
...
function metricKey(id) {
    if (id === "coordination-overhead")
        return "coordinationOverhead";
    if (id === "first-pass-success")
        return "firstPassSuccess";
    if (id === "grading-accuracy")
        return "gradingAccuracy";
    if (id === "escaped-defects")
        return "escapedDefects";
    return "costPerAcceptedCriterion";
}
function toMetricSnapshot(mv, id) {
    const base = {
        id,
        value: mv.value,
        numerator: mv.numerator,
        denominator: mv.denominator,
        sufficient: mv.sufficient,
    };
    const conf = mv.confusion;
    return conf !== undefined ? { ...base, confusion: conf } : base;
}
const GUARD_IDS = ["escaped-defects", "first-pass-success", "cost-per-accepted-criterion"];
...
async function readTrialLedger(store) {
    const applications = [];
    ...
    for (const entry of listed) {
        ...
        if (Number.isFinite(updatedAt))
            settledRunTimes.push(updatedAt);
        for (const event of state.events) {
            if (event.type !== "ownerImprovementApplicationRecorded" || event.actor !== "owner")
                continue;
            ...
            if (typeof payload.improvementProposalRef !== "string"
                ...
                || typeof payload.valueJson !== "string")
                continue;
            applications.push(Object.freeze({
                ...
                target: JSON.parse(payload.targetJson),
                value: JSON.parse(payload.valueJson),
...
export async function buildImprovementReport(store) {
    ...
    for (const proposal of proposals) {
        const matchingApplications = trialLedger.applications.filter((application) => (application.proposalHash === proposal.proposalHash));
        ...
        if (!Number.isFinite(appliedAt) || !Number.isFinite(generatedAt) || generatedAt < appliedAt)
            continue;
        const trialRecords = Object.freeze(base.records.filter((record) => Date.parse(record.recordedAt) > appliedAt));
        const trialWindow = Object.freeze({
        ...
        const currentRecommendation = trialRecommendation.recommendations.find((candidate) => (candidate.patternId === proposal.recommendation.patternId
        ...
        const mk = (id) => trialMetrics[metricKey(id)];
...
export async function buildImprovementHandoffFacts(store) {
    ...
    try {
        ...
        return { ok: false, reason: "run_unavailable" };
    }
    ...
    let state;
    try {
        state = await store.load(latest.runId);
    }
    catch {
        return { ok: false, reason: "run_unavailable" };
    }
    ...
    let planDirectory = null;
    let itemInFlight = null;
    let sessionContinuity;
    for (const event of state.events) {
        const checkpoint = checkpointPayload(event);
        if (!checkpoint)
            continue;
        if (checkpoint.resolvedPlanDirectory)
            planDirectory = checkpoint.resolvedPlanDirectory;
        else if (checkpoint.planDirectory)
            planDirectory = checkpoint.planDirectory;
        ...
        if (typeof checkpoint.runtimeStateJson === "string") {
            ...
            if (runtime.ok)
                sessionContinuity = runtime.value.sessionContinuity;
        }
    }
    const stageOrder = (stage) => RECORD_JOURNEY_CHECKPOINT_STAGES.indexOf(stage);
...
function handoffValue(value, maxLength = 512) {
    return value
        .replace(/(?:\u001b\]|\u009d)[\s\S]*?(?:\u0007|\u001b\\|\u009c)/g, "")
        .replace(/(?:\u001b\[|\u009b)[0-?]*[ -/]*[@-~]/g, "")
        .replace(/\u001b[P^_X][\s\S]*?(?:\u001b\\|\u009c)/g, "")
...
function degradationReason(reason) {
    switch (reason) {
...
function improvementHandoffNextAction(action) {
    switch (action) {
...
export function renderImprovementHandoff(facts) {
    if (!facts.degradation.ok)
        return "No degradation signal is recorded.\n";
    const stages = (values) => values.length
...
export function unnamedGreetingFor(now = new Date()) {
    if (now.getDay() === 0 || now.getDay() === 6)
        return `${salutation(now)}. Weekend warrior—let's build something great.`;
    const hour = now.getHours();
    if (hour < 5 || hour >= 22)
        return "Burning the midnight oil? What's on your mind to build?";
...
export class LocalSessionService {
    ...
    exchange(presented) {
        ...
        if (typeof presented !== "string")
            return { ok: false };
        if (presented.length !== this.capability.length)
            return { ok: false };
        if (!equalConstTime(presented, this.capability))
            return { ok: false };
        this.consumed = true;
    ...
    authenticate(cookieValue) {
        if (this.cookieValue === null)
            return false;
        if (typeof cookieValue !== "string")
            return false;
        if (cookieValue.length !== this.cookieValue.length)
            return false;
...
const NATIVE_HTML_TEMPLATE = "<!doctype html>\n" +
    '<html lang="en">\n' +
    "<head>\n" +
    '<meta charset="utf-8">\n' +
...
export function writeRejection(res, status) {
    res.writeHead(status, { "Content-Type": "text/plain; charset=utf-8" });
    res.end("Rejected");
}
function writeJourneyFailure(res, status, code) {
...
function isCapabilityBody(v) {
    if (typeof v !== "object" || v === null || Array.isArray(v) || Object.keys(v).length !== 1 || !("capability" in v))
        return false;
    const cap = v.capability;
    return typeof cap === "string" && cap.length > 0;
}
function isRepositoryPathBody(v) {
...
function isRepositoryChoiceBody(v) {
    if (typeof v !== "object" || v === null || Array.isArray(v))
        return false;
    const body = v;
    const keys = Object.keys(body);
...
function isOwnerNameBody(v) {
    if (typeof v !== "object" || v === null || Array.isArray(v) || Object.keys(v).length !== 1 || !("name" in v))
        return false;
    const name = v.name;
    return typeof name === "string" && name === name.trim() && name.length > 0 && name.length <= 80 && !/[\u0000-\u001f\u007f]/.test(name);
}
function isSelectionBody(v) {
    if (typeof v !== "object" || v === null || Array.isArray(v))
        return false;
    const body = v;
    return Object.keys(body).length === 3 && ["provider", "model", "reasoning"].every((key) => key in body) &&
        ...
        typeof body.reasoning === "string" && REASONING_LEVELS.includes(body.reasoning);
}
function hasUnsafeTextControl(value) {
    return /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/.test(value);
}
function isJourneyControlBody(v) {
    if (typeof v !== "object" || v === null || Array.isArray(v))
        return false;
    const body = v, allowed = new Set(["runId", "action", "instruction"]);
...
function isJourneyBody(v) {
    ...
    // and nowhere else.
    const stages = new Set(RECORD_JOURNEY_CHECKPOINT_STAGES);
    return Object.keys(body).every((key) => allowed.has(key)) && /^[A-Za-z0-9_-]{1,128}$/.test(String(body.runId ?? "")) &&
        stages.has(body.stage) &&
        (body.expectedRevision === undefined || (Number.isSafeInteger(body.expectedRevision) && Number(body.expectedRevision) >= 0 &&
        ...
        (body.recoveryExpectedRevision === undefined || ((body.stage === "gather-supplies" || body.stage === "execute-explorer" || body.stage === "execute-expedition") &&
        ...
        (body.retryWarrant === undefined || ((body.retryWarrant === "new_hypothesis" || body.retryWarrant === "new_evidence" || body.retryWarrant === "changed_strategy" || body.retryWarrant === "changed_environment") &&
            (body.expectedRevision !== undefined || body.recoveryExpectedRevision !== undefined)));
}
const HEADER_TOKEN = /^[!#$%&'*+\-.^_`|~0-9A-Za-z]+$/;
const QUOTED_HEADER_VALUE = /^"(?:[\t !#-\[\]-~]|\\[\t !-~])*"$/;
export function hasJsonContentType(header) {
    if (typeof header !== "string")
        return false;
    const parts = header.split(";");
    if (parts[0].trim().toLowerCase() !== "application/json")
        return false;
    for (const rawParam of parts.slice(1)) {
        const param = rawParam.trim();
        const eq = param.indexOf("=");
        if (param.length === 0 || eq <= 0)
            return false;
        const value = param.slice(eq + 1).trim();
        if (!HEADER_TOKEN.test(param.slice(0, eq).trim()))
            return false;
        if (!HEADER_TOKEN.test(value) && !QUOTED_HEADER_VALUE.test(value))
            return false;
    }
    return true;
}
/** Read a deliberately small JSON request body after the caller checks headers. */
export function readJsonBody(req, limit = MAX_SESSION_BODY) {
...
function handleSessionPost(req, res, service) {
    ...
    req.on("end", () => {
        ...
        res.setHeader("Set-Cookie", `${SESSION_COOKIE_NAME}=${result.cookieValue}; HttpOnly; SameSite=Strict; Path=/`);
        res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
        res.end("{}");
    });
    req.on("error", () => {
...
function handleRepositoryPost(req, res, service, repositoryBootstrap, repositoryChoice, selected) {
    if (!service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
    ...
    req.on("end", () => {
        ...
        const candidate = directPath
            ? Promise.resolve({ result: "selected", candidate: directPath.path, source: "picker" })
...
function handleOwnerPost(req, res, service, repositoryBootstrap, repositoryPath, repositorySelecting) {
    if (!service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
...
function repositoryFailureCode(reason) {
    const remedies = {
...
function writeRepositoryFailure(res, failure, responseStatus = "blocked") {
...
async function handleRepositoryGitignorePost(req, res, service, selected) {
    if (!service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
...
function handleRouteModelsGet(req, res, service, readiness, repositoryPath, repositorySelecting, routeId) {
    if (!service.authenticateRequest(req)) {
        writeRejection(res, 401);
        return;
    }
...
function handleReadinessPost(req, res, service, readiness, repositoryPath, repositorySelecting, remember) {
    if (!service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
    ...
    readJsonBody(req, MAX_READINESS_BODY).then(async (body) => {
        ...
        if (result.status === "ready")
            remember(result.run.roles[0].selection, result.run);
        res.writeHead(result.status === "blocked" ? 409 : 200, { "Content-Type": "application/json; charset=utf-8" });
        res.end(JSON.stringify(result));
    }, (error) => writeRejection(res, error instanceof RangeError ? 413 : 400));
}
function validatedFit(decision, resolvedPlanDirectory) {
    if (!decision
...
function confirmedFit(state, repositoryPath) {
...
async function bindConsolidationSources(repository, plan) {
...
async function duplicateConsolidation(repository, canonical) {
    const duplicate = await resolvePlanDirectory(repository, basename(canonical));
    if (duplicate.ok || duplicate.reason !== "plan_directory_ambiguous" || !duplicate.matches.includes(canonical))
        return undefined;
...
function consolidationQuestion(plan) {
...
function resultConsolidation(result) {
    if (result?.status !== "question")
        return undefined;
    const value = result.consolidation;
    if (typeof value !== "object" || value === null || !("ok" in value) || !("canonical" in value) || !("sources" in value) || !("entries" in value))
        return undefined;
    const plan = value;
...
async function fitAnswer(repositoryPath, assumption, answer) {
    if (assumption.repository !== repositoryPath)
        return { failure: "fit_undecidable" };
    ...
    if (canonical.answer === "Decline")
        return { declined: true };
    const confirmed = canonical.answer === "Confirm" || canonical.answer === assumption.planDirectory;
    let resolved;
    try {
        resolved = await resolvePlanDirectory(repositoryPath, confirmed ? assumption.planDirectory : canonical.answer);
    }
    catch {
        return {
...
function boundedJourneyTokenBudget(run) {
...
function accumulateJourneyTokens(state, result) {
...
function journeyTokenUsage(state) {
    if (state.tokenTotal === undefined
        || state.tokenBudget === undefined
        || state.tokenBudgetState === undefined)
        return undefined;
    return {
...
function recoverableFailure(result) {
...
function recoveryFitDiagnostic(result) {
    return result.code === "fit_malformed" && isFitDiagnostic(result.fitDiagnostic)
        ? { fitDiagnostic: result.fitDiagnostic }
        : {};
}
function retryRequiresWarrant(result) {
    return result.code !== "cancelled" && result.code !== "interrupted";
}
function retryErrorSignature(code) {
    if (code === "result_missing" || code === "artifact_invalid")
        return "artifact_missing";
    if (code === "focus_invalid" || code === "focus_amendment_required")
        return "git_state";
...
function retryScope(stage, code) {
    ...
    if (code === "focus_amendment_required" || code === "role_boundary_violation")
        return "contract-change";
    if (stage === "review")
        return "cross-phase";
    if (stage === "map-route" || stage === "recon" || stage === "draft-implementation")
        return "cross-slice";
    return "within-slice";
}
function escalationTargetFor(stage, result) {
    if (result?.status === "failure" && (result.code === "focus_amendment_required" || result.code === "role_boundary_violation"))
        return "owner";
    if (stage === "review")
        return "navigator";
    if (stage === "map-route" || stage === "recon" || stage === "draft-implementation")
...
function retryReasoningTier(state) {
    return state.selection
        ? normalizeReasoningTier(state.selection.reasoning, state.selection.provider) ?? "medium"
        : "medium";
}
function recordRetryDecision(state, fingerprint, scope, warrant) {
...
function clearRetryDecision(state) {
    state.retryRefusal = undefined;
    state.escalationTarget = undefined;
}
function recordedResultForStage(durable, stage) {
    const checkpoint = [...durable.events].reverse().find((event) => event.type === "journeyCheckpointRecorded" && event.payload.stage === stage);
    return parseCheckpointJson(checkpoint?.payload.lastResultJson, undefined);
}
function failedResultForStage(durable, stage, current) {
    const durableResult = recordedResultForStage(durable, stage);
    if (durableResult?.status === "failure")
        return durableResult;
    return current.stage === stage && current.lastResult?.status === "failure"
        ? current.lastResult
        : undefined;
}
function recoveryGuidance(level, code) {
    return level === "repair"
...
function journeyConcurrency(run, stage, previousStage, previous, lastResult) {
...
function journeyDisclosures(state, result = state.lastResult) {
    ...
    return {
        ...
        ...(state.sessionContinuity === "lost"
            ? { continuityLost: true, continuityDisclosure: CONTINUITY_LOST_DISCLOSURE }
            : {}),
...
function appendJourneyQa(state, question, answer) {
    state.qa.push({ question, answer });
    while (state.qa.length > 1 && Buffer.byteLength(JSON.stringify(state.qa)) > MAX_QA_JSON_BYTES)
        state.qa.shift();
}
function ensureJourneyCapacity(journeys) {
    if (journeys.size < MAX_JOURNEYS)
        return true;
    for (const [runId, state] of journeys) {
        if (!state.busy)
            journeys.delete(runId);
        if (journeys.size < MAX_JOURNEYS)
            return true;
    }
    return false;
}
function planningApprovalRecorded(state, afterRevision) {
    const reviewDecisions = new Set();
    for (const event of state.events) {
        if (event.sequence <= afterRevision)
            continue;
        if (event.type === "decisionRequired" && event.payload.question === PLAN_REVIEW_QUESTION && typeof event.payload.decisionId === "string")
            reviewDecisions.add(event.payload.decisionId);
        if (event.type === "ownerAnswered" && typeof event.payload.decisionId === "string" && reviewDecisions.has(event.payload.decisionId) && event.payload.answer === PLAN_REVIEW_APPROVAL)
            return true;
    }
    return false;
}
function focusAmendmentApprovalRecorded(state, decisionId, code = "focus_amendment_required") {
...
function reviewedPlanningValidation(state, afterRevision) {
    for (const event of [...state.events].reverse()) {
        if (event.sequence <= afterRevision || event.type !== "journeyCheckpointRecorded" || (event.payload.stage !== "map-route" && event.payload.stage !== "draft-implementation") || typeof event.payload.lastResultJson !== "string")
            continue;
        try {
            const result = JSON.parse(event.payload.lastResultJson);
            if (planningValidationSignal(result.planningValidation))
                return result.planningValidation;
        }
        catch { /* malformed checkpoint evidence is not approval evidence */ }
    }
    return undefined;
}
async function reviewedPlanningPackageCurrent(state, durable, repositoryPath, focusAmendmentConfirmed = false, presentedContentHash) {
    if (state.reviewBaselineRevision === undefined || !state.planDirectory)
        return false;
...
async function planningReviewBoundaryReached(state, durable, repositoryPath) {
    if (state.stage !== "draft-implementation"
        ...
        return false;
    return reviewedPlanningPackageCurrent(state, durable, repositoryPath, false, state.lastResult.planningValidation?.checkedContentHash);
}
async function planReviewAvailable(state, durable, repositoryPath) {
...
async function executionTransitionAllowed(state, durable, repositoryPath, stage, executionMode, reviewCadence, focusAmendmentConfirmed) {
    ...
    if ((state.stage !== "map-route" && state.stage !== "draft-implementation" && state.stage !== stage) || state.reviewBaselineRevision === undefined || mode !== expectedMode || !["slice", "phase", "end"].includes(cadence ?? ""))
        return false;
    if (!planningApprovalRecorded(durable, state.reviewBaselineRevision))
        return false;
    return reviewedPlanningPackageCurrent(state, durable, repositoryPath, focusAmendmentConfirmed === true);
}
function reviewTransitionAllowed(durable) {
    const checkpoint = durable.journeyCheckpoint;
    if (!checkpoint)
        return false;
    if (checkpoint.stage === "review")
        return true;
    if (checkpoint.stage !== "execute-explorer" && checkpoint.stage !== "execute-expedition")
        return false;
    return parseCheckpointJson(checkpoint.lastResultJson, undefined)?.status === "action";
}
function ownerAnswerRecorded(state, decisionId, answer) {
    return decisionId !== undefined && state.events.some((event) => event.type === "ownerAnswered"
        && event.payload.decisionId === decisionId
        && event.payload.answer === answer);
}
function ownerDecisionAnsweredForQuestion(state, decisionId, question, answer) {
    ...
    for (const event of state.events) {
        ...
        if (required
            && event.type === "ownerAnswered"
            && event.payload.decisionId === decisionId
            && (answer === undefined || event.payload.answer === answer))
            return true;
    }
    return false;
}
function lastQaAnswer(entries, question) {
    for (let index = entries.length - 1; index >= 0; index -= 1)
        if (entries[index].question === question)
            return entries[index].answer;
    return undefined;
}
function planningFailureReason(stage, code) {
...
function planningSignalForStage(stage) {
    return PLANNING_SIGNAL_BY_STAGE[stage];
}
function journeyStageRank(stage) {
...
function planningStateForJourneyGate(durable) {
    ...
    for (const event of durable.events) {
        ...
        if (event.payload.stage !== "recon") {
            if (event.payload.planningFailure === "OWNER_DECISION_REQUIRED")
                answeredReconOwnerDecision = false;
            continue;
        }
        const result = parseCheckpointJson(event.payload.lastResultJson, undefined);
    ...
    const planningState = answeredReconOwnerDecision && derivedPlanningState === "OWNER_DECISION_REQUIRED"
        ? "RECON_READY"
...
function retainedPlanningFailure(state) {
    switch (state) {
...
async function completedSliceEvidence(repositoryPath, runId, state, durable) {
    ...
    if (!parsed.ok || parsed.value.runId !== runId || parsed.value.planDirectory !== state.planDirectory)
        return undefined;
    ...
    if (!executionContractApprovalRecorded(durable.events, parsed.value.ownerApproval.recordId, parsed.value.contentHash))
        return undefined;
    const approvedSlices = new Map(parsed.value.slices.map((slice) => [slice.sliceId, slice]));
    const seen = new Set();
    const evidence = [];
...
async function persistJourneyCheckpoint(store, runId, state, journey, repositoryPath, expectedRevision) {
    ...
    if (!runtimeState.ok)
        throw new Error(`runtime state rejected: ${runtimeState.reason}`);
    ...
    const checkpointDiagnostic = "refused" in projected
        ? {
...
function restoreJourney(entry) {
    ...
    const qa = parseCheckpointJson(checkpoint.qaJson, []);
    if (answeredQuestion && !qa.some((item) => item.question === checkpoint.question && item.answer === entry.checkpointAnswer))
        qa.push({ question: checkpoint.question, answer: entry.checkpointAnswer });
    while (qa.length > 1 && Buffer.byteLength(JSON.stringify(qa)) > MAX_QA_JSON_BYTES)
        qa.shift();
    const savedLastResult = parseCheckpointJson(checkpoint.lastResultJson, undefined);
    const lastResult = interrupted || staleQuestion ? { status: "failure", code: "interrupted", tokens: 0 } : savedLastResult;
...
function syncBusyLease(selected) {
...
function handleJourneyPost(req, res, service, selected, journey, beforeExecutionCheckpoint) {
    ...
    if (!journey || !selected.repositoryPath || !selected.selection || !selected.run || selected.repositorySelecting) {
        writeRejection(res, 409);
        return;
    }
    ...
    readJsonBody(req, MAX_JOURNEY_BODY).then(async (value) => {
        ...
        if (!state) {
            if (!value.workGoal || !ensureJourneyCapacity(selected.journeys)) {
                writeRejection(res, 409);
                return;
            }
            ...
            state = {
                ...
                ...(tokenBudget === undefined ? {} : {
                    tokenTotal: 0,
                    tokenBudget,
                    tokenBudgetState: "within_budget",
                }),
        ...
        else if (value.workGoal && value.workGoal !== state.goal) {
            writeRejection(res, 409);
            return;
        }
        ...
        if (routeRebindRequested && value.retryWarrant !== "changed_strategy") {
            writeRejection(res, 409);
            return;
        }
        if (state.busy) {
        ...
        let durable;
        try {
            durable = await selected.store.load(value.runId);
        }
        catch {
            writeRejection(res, 503);
            return;
        }
        if (!durable.workRequestCreated) {
            writeRejection(res, 409);
            return;
        }
        if (value.expectedRevision !== undefined && durable.revision !== value.expectedRevision) {
            writeRejection(res, 409);
            return;
        }
        ...
        if (stateWasCreated)
            state.stage = durable.journeyCheckpoint?.stage ?? state.stage;
        const fitCheckpoint = [...durable.events].reverse().find((event) => event.type === "journeyCheckpointRecorded" && event.payload.repositoryFitDecision !== undefined);
        const fitResolvedPlanDirectory = fitCheckpoint?.payload.resolvedPlanDirectory;
        const durableFit = validatedFit(fitCheckpoint?.payload.repositoryFitDecision, typeof fitResolvedPlanDirectory === "string" ? fitResolvedPlanDirectory : undefined);
        if (durableFit) {
        ...
        const legacyCheckpoint = fitCheckpoint === undefined
            ? [...durable.events].reverse().find((event) => event.type === "journeyCheckpointRecorded"
        ...
        if ((value.stage === "execute-explorer" || value.stage === "execute-expedition")
            && !await executionTransitionAllowed(state, durable, repositoryPath, value.stage, value.executionMode, value.reviewCadence, value.focusAmendmentConfirmed)) {
            writeRejection(res, 409);
            return;
        }
        if (value.stage === "review" && !reviewTransitionAllowed(durable)) {
            writeRejection(res, 409);
            return;
        }
        const previousStage = stateWasCreated
            ...
            : state.stage;
        if (!journeyStageTransitionAllowed(previousStage, value.stage)) {
            writeRejection(res, 409);
            return;
        }
        ...
        if (value.recoveryExpectedRevision !== undefined
            && (durable.revision !== value.recoveryExpectedRevision || !recoveryRetry)) {
            writeRejection(res, 409);
            return;
        }
        ...
        if (warrantRecovery && value.retryWarrant !== undefined && (value.expectedRevision !== durable.revision
        ...
        const restoreCheckpointRollback = () => {
            ...
            if (checkpointRollback.concurrency)
                state.concurrency = checkpointRollback.concurrency;
            else
                delete state.concurrency;
            if (checkpointRollback.pendingRetryWarrant)
                state.pendingRetryWarrant = checkpointRollback.pendingRetryWarrant;
            else
                delete state.pendingRetryWarrant;
            if (checkpointRollback.retryRefusal)
                state.retryRefusal = checkpointRollback.retryRefusal;
            else
                delete state.retryRefusal;
            if (checkpointRollback.escalationTarget)
                state.escalationTarget = checkpointRollback.escalationTarget;
            else
                delete state.escalationTarget;
            if (checkpointRollback.recoveryOutcome)
                state.recoveryOutcome = checkpointRollback.recoveryOutcome;
            else
                delete state.recoveryOutcome;
            Object.assign(state, {
        ...
        if (retryFailure
            ...
            && value.currentSlice !== state.currentSlice)
        ...
        if (retryFailure
            ...
            if (!decision.ok || decision.escalation) {
                ...
                try {
                    if (checkpointExpectedRevision !== undefined)
                        await beforeExecutionCheckpoint?.({ runId: value.runId, expectedRevision: checkpointExpectedRevision });
        ...
        if (value.answer && value.stage !== "recon") {
            if (!state.question || state.questionStage !== value.stage) {
                writeRejection(res, 409);
                return;
            }
            ...
            if (pendingConsolidation && value.answer === CONSOLIDATION_APPROVAL) {
                if (!fitAssumption || !ownerAnswerRecorded(durable, state.questionDecisionId, value.answer)) {
                    writeRejection(res, 409);
                    return;
                }
                let current;
                try {
                    current = await bindConsolidationSources(repositoryPath, await planConsolidation(repositoryPath, pendingConsolidation.canonical, pendingConsolidation.sources));
                }
                catch {
                    writeJourneyFailure(res, 422, "input_invalid");
                    return;
                }
                ...
                let consolidation;
                try {
                    consolidation = await applyConsolidation(repositoryPath, current);
                }
                catch {
                    writeJourneyFailure(res, 422, "input_invalid");
                    return;
            ...
            if (value.stage === "gather-supplies" && state.pendingQuestions.length) {
                ...
                const result = { status: "question", question, questions: [question, ...state.pendingQuestions], tokens, ...(nextStageEstimate ? { nextStageEstimate } : {}) };
        ...
        if (value.stage === "recon" && value.answer) {
            const priorResult = state.lastResult ?? parseCheckpointJson(durable.journeyCheckpoint?.lastResultJson, undefined);
        ...
        if (value.reviewChange) {
            if (!state.planDirectory || state.question || (state.stage !== "map-route" && state.stage !== "draft-implementation")) {
                writeRejection(res, 409);
                return;
            }
        ...
        else if (value.executionMode === "explorer")
            state.currentSlice = undefined;
        if (value.cleanupMergedWorktrees !== undefined)
            appendJourneyQa(state, "Cleanup merged worktrees", value.cleanupMergedWorktrees ? "on" : "off");
        ...
        let result;
        let gatherMode = value.stage === "gather-supplies" ? value.answer || value.reviewChange || state.gatherQuestionsDiscovered ? "apply" : "questions" : undefined;
        const execute = async (reviewPrompt, gateFailureFingerprint) => {
        ...
        try {
            ...
            result = { status: "failure", code: "adapter_failed", tokens: 0 };
        }
        finally {
            state.busy = false;
            await syncBusyLease(selected).catch(() => { });
        ...
        if (result.sessionContinuity === "lost") {
            ...
            if (result.status === "failure" && result.code === "session_unavailable") {
                ...
                const continuityFailure = { status: "failure", code: "session_unavailable", tokens: 0, sessionContinuity: "lost" };
        ...
        if (result.status === "question" && result.question) {
            ...
            state.questionStage = value.stage;
            state.questionDecisionId = `journey-${randomToken(12)}`;
            state.pendingQuestions = result.questions ? [...result.questions.slice(1)] : [];
            if (value.stage === "gather-supplies" && gatherMode === "questions")
                state.gatherQuestionsDiscovered = true;
        }
        ...
        if (result.status === "action" && result.recon && result.recon.state === "OWNER_DECISION_REQUIRED") {
            ...
            result = { ...result, ownerDecisionId: id, ownerDecisionQuestion: q, ...(carriesMatchingAnswer ? { ownerDecisionAnswered: true } : {}) };
        }
        if (result.status === "action") {
            ...
            if ((value.stage === "map-route" || value.stage === "draft-implementation") && selected.store) {
                try {
                    state.reviewBaselineRevision = (await selected.store.load(value.runId)).revision;
                }
                catch {
                    result = { status: "failure", code: "adapter_failed", tokens: result.tokens };
                }
            }
        }
        state.lastResult = result;
        ...
        try {
            checkpointDiagnostic = await persistJourneyCheckpoint(selected.store, value.runId, state, journey, repositoryPath);
        }
        catch {
            writeRejection(res, 503);
            return;
...
function handleJourneyControlPost(req, res, service, selected, journey) {
    if (!service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
    ...
    if (!journey || !selected.repositoryPath) {
        writeRejection(res, 409);
        return;
    }
    readJsonBody(req, MAX_CONTROL_BODY).then((value) => {
        if (!isJourneyControlBody(value)) {
            writeRejection(res, 400);
            return;
        }
        const state = selected.journeys.get(value.runId);
        if (!state?.busy || state.control) {
            writeRejection(res, 409);
            return;
        }
...
function gitOutput(repositoryPath, args, maxBuffer = 1024 * 1024, allowDifference = false) {
...
async function gitChanges(repositoryPath) {
    ...
    for (const line of (statOutput ?? "").split("\0").filter(Boolean)) {
        ...
        if (path)
            stats.set(path, { additions: /^\d+$/.test(added) ? Number(added) : null, deletions: /^\d+$/.test(deleted) ? Number(deleted) : null });
    }
    const records = statusOutput.split("\0").filter(Boolean), changes = [];
    for (let index = 0; index < records.length && changes.length < 200; index += 1) {
        ...
        if (path)
            changes.push({ path, status, additions: stat?.additions ?? null, deletions: stat?.deletions ?? null });
        if (/[RC]/.test(status))
            index += 1;
    }
    return changes;
}
async function handleGitDiffGet(req, res, service, selected, requestedPath) {
    if (!service.authenticateRequest(req)) {
        writeRejection(res, 401);
        return;
    }
    if (!selected.repositoryPath || !requestedPath) {
        writeRejection(res, 409);
        return;
    }
...
async function handleJourneyStatusGet(req, res, service, selected, runId, journey, refreshDurable = false) {
    if (!service.authenticateRequest(req)) {
        writeRejection(res, 401);
        return;
    }
    if (!selected.repositoryPath || !selected.store) {
        writeRejection(res, 409);
        return;
    }
    if (runId && refreshDurable) {
        if (selected.journeys.get(runId)?.busy) {
            writeRejection(res, 409);
            return;
        }
    ...
    if (runId && !selected.journeys.has(runId) && !history.some((entry) => entry.runId === runId)) {
        ...
        if (created && typeof created.payload.title === "string" && typeof created.payload.goal === "string") {
            ...
            const restored = restoreJourney({ goal: created.payload.goal, updatedAt: durable.events.at(-1)?.recordedAt ?? created.recordedAt, ...(durable.pendingDecision ? { pendingQuestion: durable.pendingDecision.question } : {}), ...(answered ? { checkpointAnswer: answered.payload.answer } : {}), ...(durable.journeyCheckpoint ? { checkpoint: durable.journeyCheckpoint } : {}) });
            if (restored)
                selected.journeys.set(runId, restored);
        }
    }
    const summaries = history.map((entry) => {
        let active = selected.journeys.get(entry.runId);
        if (!active) {
            active = restoreJourney(entry);
            if (active)
                selected.journeys.set(entry.runId, active);
        }
    ...
    for (const [activeRunId, active] of selected.journeys)
        if (!storedIds.has(activeRunId))
            summaries.push({
                runId: activeRunId,
...
async function handleHistoryDelete(req, res, service, selected, runId) {
    if (!service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
    ...
    if (runId ? selected.journeys.get(runId)?.busy : [...selected.journeys.values()].some((journey) => journey.busy)) {
        writeRejection(res, 409);
        return;
    }
    if (runId) {
...
function htmlSpace(value) { return value === " " || value === "\t" || value === "\n" || value === "\r" || value === "\f"; }
function anchorTagEnd(html, start) {
    let quote = "";
...
function rewriteAnchorTag(tag, route, links) {
    const replacements = [];
    let cursor = 2;
    while (cursor < tag.length - 1) {
        while (htmlSpace(tag[cursor]))
            cursor += 1;
        if (cursor >= tag.length - 1 || tag[cursor] === "/")
            break;
        const nameStart = cursor;
        while (cursor < tag.length - 1 && !htmlSpace(tag[cursor]) && tag[cursor] !== "=" && tag[cursor] !== ">")
            cursor += 1;
        ...
        if (quote !== '"' && quote !== "'") {
            while (cursor < tag.length - 1 && !htmlSpace(tag[cursor]) && tag[cursor] !== ">")
                cursor += 1;
            continue;
        }
        ...
        if (name === "href") {
            ...
            if (artifactIndex !== undefined)
                replacements.push({ start: valueStart, end: valueEnd, value: `${route}/${artifactIndex}` });
        }
        cursor = valueEnd + 1;
    }
...
function rewriteArtifactLinks(html, route, links) {
    ...
    while (search < html.length) {
        ...
        if (!htmlSpace(boundary) && boundary !== ">" && boundary !== "/") {
            search = start + 2;
            continue;
        }
...
async function handleJourneyArtifactGet(res, service, req, selected, runId, indexText) {
    if (!service.authenticateRequest(req) || !selected.repositoryPath) {
        writeRejection(res, 401);
        return;
    }
    ...
    try {
        ...
        try {
            ...
            if (!opened.isFile() || linked.isSymbolicLink() || !linked.isFile() || opened.dev !== linked.dev || opened.ino !== linked.ino || opened.size > MAX_JOURNEY_ARTIFACT || !relation || relation.startsWith("..") || isAbsolute(relation))
                throw new Error("invalid artifact");
            ...
            if (length > MAX_JOURNEY_ARTIFACT)
                throw new Error("invalid artifact");
        ...
        if (html) {
            const directory = posix.dirname(path), links = new Map();
            state?.artifacts.forEach((artifactPath, artifactIndex) => {
                const relativeArtifact = posix.relative(directory, artifactPath);
                if (/^[A-Za-z0-9][A-Za-z0-9._/-]*\.(?:md|html)$/i.test(relativeArtifact) && !relativeArtifact.split("/").includes("..") && !links.has(relativeArtifact))
                    links.set(relativeArtifact, artifactIndex);
            });
            ...
            if (artifact.length > MAX_JOURNEY_ARTIFACT)
                throw new Error("invalid artifact");
        }
...
function writeShowcaseJson(res, value) {
...
function writeRunReadFailure(res, code) {
    writeRepositoryFailure(res, runReadFailure(code));
}
function writeRunReadJson(res, value) {
    const body = JSON.stringify(value);
    if (Buffer.byteLength(body) > MAX_SHOWCASE_JSON) {
...
function writeImprovementFailure(res, code) {
    const remedies = {
...
async function handleImprovementReportGet(req, res, service, selected, report) {
    if (req.headers.origin !== undefined && !service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
...
function isOwnerApplicationRecordBody(v) {
    if (!isRecord(v)
        || Object.keys(v).length !== 6
        || !["runId", "proposalHash", "surface", "target", "value", "externalEvidenceHash"].every((key) => Object.hasOwn(v, key))
        || typeof v.runId !== "string"
        || !/^[A-Za-z0-9_-]{1,128}$/.test(v.runId))
        return false;
    if (typeof v.proposalHash !== "string" || !/^[a-f0-9]{64}$/.test(v.proposalHash))
        return false;
    if (typeof v.surface !== "string" || v.surface.length === 0 || v.surface.length > 128)
        return false;
    if (typeof v.externalEvidenceHash !== "string" || !/^[a-f0-9]{64}$/.test(v.externalEvidenceHash))
        return false;
...
async function handleOwnerImprovementApplication(req, res, service, selected, report) {
    if (req.headers.origin !== undefined && !service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
    ...
    if (!isOwnerApplicationRecordBody(body)) {
        writeRejection(res, 400);
        return;
    }
    if (!selected.store) {
    ...
    if (selected.inFlightImprovementProposals.has(body.proposalHash)) {
        writeRejection(res, 409);
        return;
    }
    ...
    try {
        ...
        const reportValue = reportRes.value;
...
function writeImprovementHandoffFailure(res, code) {
    const failures = {
...
async function handleImprovementHandoffGet(req, res, service, selected) {
    if (req.headers.origin !== undefined && !service.validOrigin(req.headers.origin)) {
...
async function executionContractSource(repositoryPath, planDirectory) {
    ...
    try {
        ...
        try {
            return { available: true, value: JSON.parse(content) };
        }
        catch {
...
export function executionContractApprovalRecorded(events, recordId, contentHash) {
 ...
 */
async function resolveApprovedExecutionContract(repositoryPath, runId, durable) {
    ...
    if (typeof planDirectory !== "string")
 ...
 */
async function executionContractFileAbsent(repositoryPath, durable) {
    ...
    if (durable.events.some((event) => event.type === "ownerAnswered"
        && event.actor === "owner"
        && event.payload.answer === PLAN_REVIEW_APPROVAL
        && typeof event.payload.ownerApprovedContentHash === "string"))
        return false;
    const planDirectory = [...durable.events].reverse().find((event) => event.type === "journeyCheckpointRecorded"
        && typeof event.payload.planDirectory === "string")?.payload.planDirectory;
    if (typeof planDirectory !== "string")
 ...
 */
async function legacyBindingAdmitted(repositoryPath, durable, contract) {
    if (contract.ok || contract.code !== CONTRACT_ABSENT)
        return false;
    if (isCompactedRunState(durable))
        return false;
    return executionContractFileAbsent(repositoryPath, durable);
}
async function approvedExecutionContractForRun(res, repositoryPath, runId, durable) {
    const resolved = await resolveApprovedExecutionContract(repositoryPath, runId, durable);
    if (!resolved.ok) {
        writeRunReadFailure(res, resolved.code);
        return undefined;
    }
    return resolved.value;
}
async function handleExecutionContractGet(req, res, service, selected, runId, sliceId) {
    if (req.headers.origin !== undefined && !service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
    ...
    let durable;
    try {
        durable = await selected.store.load(runId);
    }
    catch {
    ...
    if (!durable.workRequestCreated) {
        writeRunReadFailure(res, "run_not_found");
        return;
    }
    ...
    if (slice && role !== "explorer" && role !== "navigator" && role !== "crewmate") {
        writeRunReadFailure(res, "slice_not_projectable");
        return;
    }
    ...
    if ("ok" in focusEnvelope && !focusEnvelope.ok) {
        writeRunReadFailure(res, focusEnvelope.reason);
        return;
    }
    writeRunReadJson(res, { contract: parsed.value, focusEnvelope, advisories: parsed.advisories });
}
async function handlePlanningStateGet(req, res, service, selected, runId) {
    if (req.headers.origin !== undefined && !service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
...
function parseVerificationCheckpoint(value) {
...
async function handleVerificationReportGet(req, res, service, selected, runId, layer) {
    if (req.headers.origin !== undefined && !service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
    ...
    const entries = [];
    for (const event of durable.events) {
        if (event.type !== "journeyCheckpointRecorded" || event.payload.verification === undefined)
            continue;
        const parsed = parseVerificationCheckpoint(event.payload.verification);
        if (!parsed.ok) {
            writeRunReadFailure(res, parsed.reason);
            return;
        }
        if (parsed.value.layer !== layer)
            continue;
        const stage = event.payload.stage;
        const status = event.payload.status;
        if (typeof stage !== "string"
            || !RECORD_JOURNEY_CHECKPOINT_STAGES.includes(stage)
...
async function handleVerificationReportPost(req, res, service, selected, runId, layer) {
    ...
    if (!hasJsonContentType(req.headers["content-type"])) {
        writeRunReadFailure(res, "verification_content_type_unsupported");
        return;
    }
    let body;
    try {
        body = await readJsonBody(req, MAX_VERIFICATION_BODY);
    }
    catch (error) {
    ...
    const independence = {
        implementerSessionIds: [...new Set(durable.events.flatMap((event) => typeof event.payload.providerSessionId === "string" ? [event.payload.providerSessionId] : []))],
        executionAncestry: [],
    };
    if (layer === "grader") {
        ...
        if (!independent.ok) {
            writeRunReadFailure(res, independent.code);
            return;
        }
        ...
        if (reports.length === 0 || reports.length > 128) {
            writeRunReadFailure(res, "malformed");
            return;
        }
        const claimSource = [...durable.events].reverse().find((event) => event.type === "journeyCheckpointRecorded"
        ...
        if (claimSource) {
            try {
                const result = JSON.parse(claimSource.payload.lastResultJson);
        ...
        if (refused && !refused.ok) {
            writeRunReadFailure(res, refused.reason);
            return;
        }
    ...
    if (!durable.journeyCheckpoint) {
        writeRunReadFailure(res, "verification_checkpoint_rejected");
        return;
    }
    ...
    if (recorded.ok && !event) {
        try {
            event = (await selected.store.load(runId)).events.find((candidate) => candidate.causationId === commandId);
        }
        catch {
            writeRunReadFailure(res, "run_unavailable");
            return;
...
async function handleReviewCadenceGet(req, res, service, selected, runId) {
    if (req.headers.origin !== undefined && !service.validOrigin(req.headers.origin)) {
        writeRejection(res, 403);
        return;
    }
 ...
 */
export function createRequestHandler(service, repositoryBootstrap = new RepositoryBootstrap(), options = {}) {
    const selected = {
    ...
    const readiness = new ReadinessService(options.routeInspection ?? options.processRunner ?? { executableAvailable: () => false }, options.verification ?? (options.processRunner ? new AdapterVerification(options.processRunner) : undefined), options.startupOverrides);
    ...
    const improvementReport = Object.hasOwn(options, "improvementReport")
        ...
        : "improvementReport" in options
            ? async () => ({ ok: false, reason: "configuration_invalid" })
            : undefined;
    return (req, res) => {
        ...
        if (method === "GET" && path.startsWith("/api/v1/git-diff?")) {
            ...
            if (!requestedPath)
                ...
                void handleGitDiffGet(req, res, service, selected, requestedPath).catch(() => writeRejection(res, 500));
            return;
        }
        ...
        if (method === "POST" && verificationReport && verificationReport[2] !== "validator") {
            void handleVerificationReportPost(req, res, service, selected, verificationReport[1], verificationReport[2])
                .catch(() => writeRunReadFailure(res, "run_unavailable"));
            return;
        }
        if (method === "GET" && verificationReport) {
            void handleVerificationReportGet(req, res, service, selected, verificationReport[1], verificationReport[2]).catch(() => writeRejection(res, 500));
            return;
        }
        ...
        if (method === "GET" && path === "/api/v1/workflows") {
            if (selected.store === null)
                ...
                writeShowcaseJson(res, { schemaVersion: 1, workflows: listWorkflowShowcases() });
            return;
        }
        ...
        if (method === "GET" && report) {
            if (selected.store === null)
                ...
                if (html === null)
                    writeRejection(res, 404);
                else if (Buffer.byteLength(html) > MAX_SHOWCASE_REPORT)
                    writeRejection(res, 500);
                else {
                    res.writeHead(200, {
        ...
        if (method === "GET" && workflow) {
            if (selected.store === null)
                ...
                if (projection === null)
                    ...
                    writeShowcaseJson(res, projection);
            }
            return;
        }
        const command = /^\/api\/v1\/runs\/([A-Za-z0-9_-]{1,128})\/commands$/.exec(path);
        if (method === "POST" && command) {
            if (selected.gateway === null)
                writeRejection(res, 409);
            else
                selected.gateway.handle(req, res, command[1]);
            return;
        }
        ...
        if (method === "GET" && run) {
            if (selected.gateway === null)
                ...
                selected.gateway.read(req, res, run[1]);
            return;
        }
        const events = /^\/api\/v1\/runs\/([A-Za-z0-9_-]{1,128})\/events$/.exec(path);
        if (method === "GET" && events) {
            if (selected.sse === null)
                writeRejection(res, 409);
            else
                selected.sse.handle(req, res, events[1]);
            return;
        }
        writeRejection(res, 404);
    };
 ...
 */
export async function admitRepositoryRoot(repository) {
    ...
    try {
        ...
        if (repositoryPath !== (repository.replace(/[/\\]+$/, "") || repository))
            return undefined;
        ...
        if (!(assessRepositorySafety({
            ...
            return undefined;
        if ((await readdir(repositoryPath)).some((entry) => entry.startsWith(".bearing.tmp-")))
            return undefined;
    }
    catch {
        return undefined;
    ...
    try {
        ...
        if (!bearing.isDirectory() || bearing.isSymbolicLink())
            return undefined;
    }
    catch (error) {
...
async function freshHeadlessRead(repository) {
...
function invokeHeadlessHandler(handler, method, path, headers, body) {
        ...
        const response = {
            writeHead: (nextStatus, nextHeaders) => {
                ...
                for (const [name, value] of Object.entries(nextHeaders ?? {}))
                    responseHeaders[name.toLowerCase()] = Array.isArray(value) ? value.map(String) : String(value);
                return response;
            },
            setHeader: (name, value) => { responseHeaders[name.toLowerCase()] = Array.isArray(value) ? value : value; },
            end: finish,
        };
...
function headlessAmendmentCode(run) {
...
function isIssue85Recovery(run) {
...
function isRetryWarrantRecovery(run) {
    if (!run || (run.status !== "failed" && run.status !== "stopped"))
        return false;
    const lastResult = typeof run.lastResult === "object" && run.lastResult !== null && !Array.isArray(run.lastResult)
        ? run.lastResult
...
function headlessFailedRecoveryDecision(run, pendingDecision) {
...
function headlessRecoveryPlan(run, pendingDecision, readinessReady) {
    if (!run)
        return { allowedActions: ["create", "resume", "status"] };
    ...
    const lastResult = typeof run.lastResult === "object" && run.lastResult !== null && !Array.isArray(run.lastResult)
        ...
        : undefined;
    if (!readinessReady)
        return { allowedActions: isIssue85Recovery(run) ? ["status"] : ["status", "resume"] };
    ...
    if (headlessFocusAmendmentRequired(run))
        return { allowedActions: ["status", "resume", "confirm-amendment"] };
    const issue85Recovery = isIssue85Recovery(run);
    if (issue85Recovery && run.recoveryAvailable !== true)
        return { allowedActions: ["status"] };
    const warrantRecovery = isRetryWarrantRecovery(run);
    if (warrantRecovery && run.recoveryAvailable !== true)
        return { allowedActions: ["status"] };
    if ((run.status === "failed" || run.status === "stopped") && run.recoveryAvailable === true) {
        ...
        if (warrantRecovery && typeof run.stage === "string" && RECORD_JOURNEY_CHECKPOINT_STAGES.includes(run.stage)) {
            return {
    ...
    if (run.status === "failed" || run.status === "stopped")
        return { allowedActions: ["status", "resume", "progress"] };
...
const HEADLESS_FINDING_CODES = new Set(["artifact_frontmatter_invalid", "design_section_missing", "seit_section_missing"]);
const HEADLESS_FINDING_ARTIFACTS = new Set(["design.md", "seit.md"]);
 ...
 */
function headlessFindings(value) {
    if (!Array.isArray(value))
        return [];
    const projected = [];
    for (const candidate of value) {
        if (projected.length === MAX_HEADLESS_FINDINGS)
            break;
        if (typeof candidate !== "object" || candidate === null || Array.isArray(candidate))
            continue;
        const finding = candidate;
        if (typeof finding.code !== "string" || !HEADLESS_FINDING_CODES.has(finding.code))
            continue;
        if (typeof finding.artifact !== "string" || !HEADLESS_FINDING_ARTIFACTS.has(finding.artifact))
            continue;
        if (finding.severity !== "advisory" && finding.severity !== "amendment")
            continue;
        if (!headlessText(finding.observed) || !headlessText(finding.required) || !headlessText(finding.remedy))
            continue;
 ...
 */
function browserLastResult(lastResult) {
    if (lastResult === undefined || lastResult.status !== "failure" || lastResult.code !== "artifact_invalid" || lastResult.findings === undefined)
        return lastResult;
    return { ...lastResult, findings: headlessFindings(lastResult.findings) };
}
function headlessSelectedScope(value) {
    ...
    if (typeof currentSlice !== "string" || !/^(?:[A-Za-z]+\d+|\d+(?:\.\d+)+)$/.test(currentSlice))
        return undefined;
    if (!Array.isArray(remainingSlices) || remainingSlices.length !== 1 || remainingSlices[0] !== currentSlice)
        return undefined;
    if (!Array.isArray(allowedPaths) || allowedPaths.length > MAX_HEADLESS_ARTIFACTS || !allowedPaths.every((path) => typeof path === "string" && repositoryRelativePlanDirectory(path)))
        return undefined;
    if (!Array.isArray(seitCommandIds) || seitCommandIds.length > MAX_HEADLESS_ARTIFACTS || !seitCommandIds.every((id) => typeof id === "string" && /^[A-Za-z0-9._-]{1,128}$/.test(id)))
        return undefined;
...
function projectHeadlessReceipt(run, pendingDecision, readinessReady) {
    ...
    return {
        ...
        ...(fitCorrection ? { validationError: "repository_fit_answer_invalid", remedy: FIT_OWNER_ANSWER_REMEDY, correctionAction: "decide" } : {}),
        ...(summary ? { summary } : {}),
        ...
        ...(focusAmendmentRequired
            ? { requiredOwnerAction: { type: "confirm-amendment", prompt: headlessAmendmentPrompt(run) } }
            : {}),
        ...(outcome ? { outcome } : {}),
 ...
 */
export async function readDurableContinuation(repository, runId) {
    const repositoryPath = await admitRepositoryRoot(repository);
    if (repositoryPath === undefined)
    ...
    const summary = isCompactedRunState(durable)
        ...
        : created && typeof created.payload.goal === "string"
            ...
            : undefined;
    if (!summary)
        return { ok: false, code: "run_not_found", repositoryPath };
    const checkpoint = durable.journeyCheckpoint;
    ...
    const state = restoreJourney({
        ...
        ...(answered ? { checkpointAnswer: answered.payload.answer } : {}),
    ...
    const projection = projectHeadlessReceipt(run, durable.pendingDecision, routeSaved);
    const planDirectory = state?.resolvedPlanDirectory ?? state?.planDirectory;
    const blockers = new Set();
    if (state?.lastResult?.status === "failure" && headlessText(state.lastResult.code))
        blockers.add(state.lastResult.code);
    if (state?.retryRefusal)
        blockers.add(state.retryRefusal);
    if (checkpoint?.planningFailure)
        blockers.add(checkpoint.planningFailure);
    if (!routeSaved)
        blockers.add("route_unavailable");
 ...
 */
export async function applyLegacyRoleRouteApproval(request) {
    ...
    let durable;
    try {
        durable = await store.load(request.runId);
    }
    catch {
        return { ok: false, code: "store_unreadable" };
    }
    if (!durable.workRequestCreated)
        return { ok: false, code: "run_not_found" };
    if (!roleRoutesShape(request.roleRoutes))
        return { ok: false, code: "role_routes_invalid", revision: durable.revision };
    ...
    if (contract.ok)
        return { ok: false, code: "execution_contract_authoritative", revision: durable.revision };
    ...
    if (contract.code !== CONTRACT_ABSENT)
        return { ok: false, code: contract.code, revision: durable.revision };
    ...
    if (durable.revision !== request.expectedRevision)
        return { ok: false, code: "stale_revision", revision: durable.revision };
 ...
 */
export async function applyLegacyExecutionContractApproval(request) {
    const repositoryPath = await admitRepositoryRoot(request.repository);
    if (repositoryPath === undefined)
        return { ok: false, code: "repository_rejected" };
    const store = new BearingStore(repositoryPath);
    ...
    const applied = await store.apply({
        ...
        payload: { contract: parsed.value, approvedContentHash: request.approvedContentHash },
    }).catch(() => undefined);
    if (!applied?.ok) {
        return applied?.reason === "stale_revision"
            ? { ok: false, code: "stale_revision", revision: durable.revision }
 ...
 */
function implementerIdentities(durable) {
 ...
 */
async function reviewFacts(repository, runId) {
    const repositoryPath = await admitRepositoryRoot(repository);
    if (repositoryPath === undefined)
        return { ok: false, code: "repository_rejected" };
    const store = new BearingStore(repositoryPath);
    ...
    if (!contract.ok)
        return { ok: false, code: contract.code };
    const snapshot = await snapshotGitState(repositoryPath);
    if (!snapshot || snapshot.head === null || snapshot.paths.size > 0)
        return { ok: false, code: "candidate_unclean" };
    return {
        ok: true,
        value: {
            repositoryPath,
...
export async function readReviewContext(repository, runId, reviewClass) {
 ...
 */
export async function recordReviewGate(request) {
    const found = await reviewFacts(request.repository, request.runId);
    if (!found.ok)
        return found;
    const facts = found.value;
    if (request.reviewedRevision !== facts.reviewedRevision)
        return { ok: false, code: "revision_changed", revision: facts.durable.revision };
    if (request.contractHash !== facts.contract.contentHash)
        return { ok: false, code: "contract_mismatch", revision: facts.durable.revision };
    const scope = [...new Set(request.scope)].sort();
    if (canonicalStringify(scope) !== canonicalStringify(facts.scope))
        return { ok: false, code: "scope_mismatch", revision: facts.durable.revision };
    ...
    if (!isReviewRecord(record))
        return { ok: false, code: "verdict_unsupported", revision: facts.durable.revision };
    ...
    if (facts.durable.revision !== request.expectedRevision)
        return { ok: false, code: "stale_revision", revision: facts.durable.revision };
    if (facts.durable.pendingDecision !== null)
        return { ok: false, code: "pending_decision_blocks", revision: facts.durable.revision };
...
export async function executeHeadlessJourney(request, deps) {
    ...
    const journeyStatusRefreshes = new Set();
    ...
    if (session.status !== 200 || typeof cookie !== "string")
        return { ok: false, code: "transition_unavailable", runId: request.runId, revision: 0 };
    ...
    if (!readinessReady && !readAction)
        return { ok: false, code: "route_unavailable", runId: request.runId, revision: 0, ...firstLaunch };
    const readRun = async () => {
    ...
    const restore = (refreshDurable = false) => {
        if (refreshDurable)
            journeyStatusRefreshes.add(request.runId);
    ...
    const snapshotAttempt = async (action, refreshDurable = false) => {
    ...
    const stableSnapshot = async (action) => {
        const initial = await snapshotAttempt(action);
        if (action !== "status" || initial.stable || isIssue85Recovery(initial.run))
            return initial;
    ...
    const command = async (type, payload, expectedRevision) => {
    ...
    if (request.action === "create") {
        if (!request.goal)
            return { ok: false, code: "input_invalid", runId: request.runId, revision: 0 };
        ...
        const waitingStage = restoredRun?.status === "waiting"
            ...
            && RECORD_JOURNEY_CHECKPOINT_STAGES.includes(restoredRun.stage)
            ? restoredRun.stage
            : undefined;
        ...
        const reviewAvailable = typeof restored === "object"
            ...
            && restored.planReviewAvailable === true;
        let current = await readRun();
        const pending = current.body?.pendingDecision;
        if (!reviewAvailable || !current.body || (pending !== null && (typeof pending !== "object" || pending.question !== PLAN_REVIEW_QUESTION))) {
            operation = { status: 409, body: "", headers: {} };
        }
        else {
            if (pending === null) {
                ...
                if (required.status !== 200) {
                    ...
                    operation = !current.body || typeof created !== "object" || created === null || created.question !== PLAN_REVIEW_QUESTION || typeof created.decisionId !== "string"
                        ? { status: 409, body: "", headers: {} }
                        : await command("recordOwnerAnswer", { decisionId: created.decisionId, answer: PLAN_REVIEW_APPROVAL });
                }
            }
            else {
                operation = await command("recordOwnerAnswer", { decisionId: pending.decisionId, answer: PLAN_REVIEW_APPROVAL });
            }
        }
    }
        ...
        if (stage === undefined || current.body?.pendingDecision !== null || !headlessFocusAmendmentRequired(restoredRun)) {
            ...
            if (required.status !== 200) {
                ...
                current = await readRun();
                const pending = current.body?.pendingDecision;
                if (!current.body || typeof pending !== "object" || pending === null || pending.decisionId !== decisionId || pending.question !== amendmentPrompt) {
                    operation = { status: 409, body: "", headers: {} };
                }
                else {
        ...
        const advertisedWarrant = recovery?.type === "progress"
            && recovery.retryWarrants?.includes(request.retryWarrant) === true;
                    ...
                    : isRetryWarrantRecovery(snapshot.run)
                        ? (recovery?.type === "progress" && recovery.stage === stage || routeStrategyRecovery)
                            ...
                            ? await invokeHeadlessHandler(handler, "POST", "/api/v1/journey", authenticated, {
                                ...
                                ...(stage === "execute-expedition" && (request.currentSlice ?? (typeof snapshot.run?.currentSlice === "string" ? snapshot.run.currentSlice : undefined)) ? { currentSlice: request.currentSlice ?? snapshot.run?.currentSlice } : {}),
                            })
                            : { status: 409, body: "", headers: {} }
                        : request.retryWarrant !== undefined
                            ? { status: 409, body: "", headers: {} }
    ...
    const run = typeof state === "object" && state !== null && !Array.isArray(state) ? state : undefined;
    const durable = statusSnapshot?.after ?? await readRun();
    const stage = typeof run?.stage === "string" && RECORD_JOURNEY_CHECKPOINT_STAGES.includes(run.stage) ? run.stage : undefined;
    const status = run?.status;
    const recoveryPlan = headlessRecoveryPlan(run, durable.body?.pendingDecision, readinessReady);
