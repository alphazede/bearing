...
function safeImproveDestination(value) {
...
export function parseImproveArgs(args) {
    if (!Array.isArray(args)
        ...
        || !Array.from({ length: args.length }, (_, index) => index)
            .every((index) => Object.hasOwn(args, index)))
        return { ok: false };
    if (args[0] !== "improve")
        return { ok: false };
...
const JOURNEY_ACTIONS = new Set(["create", "resume", "status", "decide", "approve-route", "select-explorer", "progress"]);
const JOURNEY_STAGES = new Set(RECORD_JOURNEY_CHECKPOINT_STAGES);
const JOURNEY_REASONING = new Set(REASONING_VALUES);
function safeJourneyText(value) {
...
export function parseJourneyArgs(args) {
    if (!Array.isArray(args) || args[0] !== "journey" || !JOURNEY_ACTIONS.has(args[1]))
        return { ok: false };
    const values = new Map();
    for (let index = 2; index < args.length; index += 2) {
        const name = args[index], value = args[index + 1];
        if (!/^--(?:repo|provider|model|reasoning|run|goal|answer|review-cadence|stage)$/.test(name ?? "") || value === undefined || value.startsWith("--") || values.has(name))
            return { ok: false };
        values.set(name, value);
    }
    const action = args[1];
...
async function readPlanSource(root, path) {
    ...
    try {
        ...
        if (!opened.isFile() || linked.isSymbolicLink() || !linked.isFile() || opened.dev !== linked.dev || opened.ino !== linked.ino || opened.size > MAX_PLAN_SOURCE_BYTES || !relation || relation.startsWith("..") || isAbsolute(relation))
            return undefined;
    ...
    if (args[1] === "compact" || args[1] === "prune") {
        ...
        for (let index = 2; index < args.length;) {
            ...
            if (!value || value.startsWith("--"))
                return undefined;
            if (flag === "--repo") {
                if (repository !== undefined || !isAbsolute(value))
                    return undefined;
                repository = value;
                continue;
            }
            const parsed = nonNegativeInteger(value);
            if (parsed === undefined || command !== "prune")
                return undefined;
        ...
        if (maxAgeDays === undefined && maxCompletedRuns === undefined)
            return undefined;
        return {
            command,
            policy: {
    ...
    for (let index = 2; index < args.length;) {
        ...
        if (flag === "--scan") {
            ...
            while (index < args.length && !args[index].startsWith("--")) {
                if (!isAbsolute(args[index]))
                    return undefined;
                scans.push(args[index++]);
            }
            if (scans.length === start)
                return undefined;
            continue;
        }
        const value = args[index++];
        if (flag !== "--relocate" || relocate !== undefined || !value || !isAbsolute(value))
            return undefined;
        relocate = value;
    }
...
export function parseStartArgs(args) {
    ...
    const reasoning = typeof rawReasoning === "string" ? normalizeReasoningTier(rawReasoning, typeof values.get("provider") === "string" ? values.get("provider") : undefined) : undefined;
...
const DEFAULT_IMPROVEMENT_CLI_DEPS = Object.freeze({});
...
function recommendationDescription(recommendation) {
...
function improvementStatusLines(report) {
    return [
        "Improvement status",
...
function improvementReportLines(report) {
    ...
    if (report.recommendation.recommendations.length === 0)
    ...
    if (report.trialVerdicts.length === 0)
        lines.push("Trial verdicts: none");
...
function contributionTarget(recommendation) {
    const target = recommendation.target;
    if (Object.hasOwn(target, "role") && typeof target.role === "string")
        return target.role;
    if (Object.hasOwn(target, "layer") && typeof target.layer === "string")
        return target.layer;
    if (Object.hasOwn(target, "scope") && typeof target.scope === "string")
        return target.scope;
    ...
    if (Object.hasOwn(target, "sectionId") && typeof target.sectionId === "string")
        return target.sectionId;
    return "invalid/target";
}
function contributionAtom(value) {
    if (value === null)
        return "none";
    if (typeof value === "string" || typeof value === "number")
        return value;
...
function improvementStore(improvement, repositoryRoot) {
    if (Object.hasOwn(improvement, "storeFactory")) {
        if (typeof improvement.storeFactory !== "function")
 ...
 */
async function exportFailureDetail(repositoryRoot, destination) {
...
async function loadImprovementReport(improvement, repositoryRoot) {
...
export function run(args, deps = {}) {
    ...
    if (args[0] === "improve") {
        ...
        if (parsed.action === "export") {
            const operation = Object.hasOwn(improvement, "export")
    ...
    if (args[0] === "journey") {
        ...
        const transition = deps.headlessJourney ?? ((request) => executeHeadlessJourney(request, {
        ...
        return transition(parsedJourney).then((receipt) => {
            ...
            stdout.write(`${JSON.stringify({ ok: false, code: "transition_unavailable", runId: parsedJourney.runId, revision: 0 })}\n`);
            exit(1);
            return undefined;
        });
    }
