...
import { artifactComplete, parsePlanDocuments, requiredSystemSpecFields, sectionPresent, structuralFindings, writeSetPathIssue, } from "./plan-structure.js";
...
const COMMAND_BINDING_REQUIRED = "every declared command ID must resolve to an exact repository command or an explicitly typed external procedure";
 ...
 */
function exactRepositoryCommand(binding) {
    const literals = [...binding.matchAll(/`([^`]+)`/g)].map((match) => match[1]);
    return literals.some((literal) => literal.trim().length > 0
 ...
 */
function explicitExternalProcedure(id, binding) {
    if (!id.startsWith("PROC-"))
        return false;
    const prose = binding.replace(/`[^`]*`/g, " ").replace(/[*_~]/g, "").trim();
    if (!prose || prose.length > MAX_RATIONALE || COMMAND_PLACEHOLDER.test(prose))
        return false;
    return prose.split(/\s+/).length >= 4;
}
function requiredCommandEntries(seitDocument) {
 ...
 */
function commandIsBound(id, binding) {
    return exactRepositoryCommand(binding) || explicitExternalProcedure(id, binding);
}
function commandBindingFindings(model) {
    const findings = [];
    const entries = requiredCommandEntries(model.documents.seit);
    for (const [id, binding] of entries) {
        if (commandIsBound(id, binding))
            continue;
        findings.push(finding("command_unbound", "amendment", "seit.md", `${id}: ${binding || "(no binding)"}`, COMMAND_BINDING_REQUIRED, "bind the command to an exact repository command in backticks or an explicitly typed external procedure description"));
    }
    for (const [id, manifest] of model.manifests) {
        for (const command of manifest.commandIds) {
            ...
            if (!model.requiredCommands.has(normalized))
                continue;
            if (commandIsBound(normalized, entries.get(normalized) ?? ""))
                continue;
            findings.push(finding("command_unbound", "amendment", "implementation.md", command, COMMAND_BINDING_REQUIRED, "bind the command to an exact repository command in the Required Commands section", id));
        }
    }
    return findings;
}
...
const DEPENDENCY_NEGATION = /\b(?:no|not|never|without|no\s+longer|unchanged|untouched|avoid(?:ing)?|skip(?:ping)?|forbid(?:den)?|prohibit(?:ed)?)\b|\b(?:don['’]t|won['’]t|mustn['’]t|shouldn['’]t|can['’]t|cannot)\b/i;
 ...
 * fabricate intent.
 */
function declaresDependencyChange(text) {
    let inCode = false;
    let masked = "";
    ...
    for (const { clause, maskedClause } of clauses) {
        ...
        if (!verbMatch || verbMatch.index === undefined || Math.abs(verbMatch.index - nounIndex) > DEPENDENCY_PROXIMITY)
            continue;
...
const DEPENDENCY_LOCKFILE_PATH = /(?:^|\/)(?:pnpm-lock\.yaml|package-lock\.json|yarn\.lock|npm-shrinkwrap\.json|bun\.lock|bun\.lockb|cargo\.lock|poetry\.lock|pipfile\.lock|gemfile\.lock|composer\.lock|deno\.lock|go\.sum)$/i;
const DEPENDENCY_OWNERSHIP_REQUIRED = "slices that introduce package dependency changes must own a package manifest or lockfile in their write set";
/**
 ...
 */
function dependencyOwnershipFindings(model) {
    const findings = [];
    for (const [id, manifest] of model.manifests) {
        ...
        if (ownsManifest || ownsLockfile)
            continue;
        findings.push(finding("dependency_unowned", "amendment", "implementation.md", declared, DEPENDENCY_OWNERSHIP_REQUIRED, "add the package manifest and lockfile to the slice write set or move the dependency change to an owning slice", id));
    }
    return findings;
}
...
const PROCEDURE_MISMATCH_REQUIRED = "every executable-procedure traceability row must resolve to exactly one procedure narrative whose command, positive case, negative case, and evidence target match the row";
function procedureFindings(model) {
    const findings = [];
    ...
    if (!model.procedureNarratives.size)
        return findings;
    for (const [rowId, narratives] of model.procedureNarratives) {
        if (model.traceRows.has(rowId))
            continue;
...
const SYSTEM_SPEC_REQUIRED = "every System Catalog entry must resolve to exactly one per-system specification with non-empty Ownership, Inputs, Outputs, APIs, Data ownership, Invariants, Trust boundary, Failure modes, and Observability fields";
const SYSTEM_TRACE_REQUIRED = "every declared requirement must reach a Requirement Trace row, and every requirement, system, contract, SEIT row, slice, and path in the trace must resolve to declared plan content";
function systemFindings(model) {
    const findings = [];
...
const SYSTEM_OWNER_BINDING_REQUIRED = "every Requirement Trace row must bind each requirement to a slice that declares it in Requirement IDs";
const SYSTEM_SCOPE_BINDING_REQUIRED = "every Requirement Trace row must bind each contract and SEIT row to a slice that declares it in Design IDs and SEIT proof rows";
const SYSTEM_PATH_BINDING_REQUIRED = "every Requirement Trace row must bind each path to a slice whose write set covers it";
/**
 ...
 */
function systemBindingFindings(model) {
    const findings = [];
    const producedPaths = new Set([...model.manifests.values()].flatMap((manifest) => manifest.writeSetPaths));
    for (const row of model.systemTraceRows) {
        ...
        for (const requirement of row.requirements) {
            if (named.some((id) => model.slices.get(id)?.requirementIds.has(requirement)))
                continue;
            findings.push(finding("system_owner_conflict", "amendment", "design.md", `${requirement}: Requirement Trace row names slice(s) ${label}, none declaring the requirement`, SYSTEM_OWNER_BINDING_REQUIRED, `declare ${requirement} in the named slice's Requirement IDs or fix the trace row`));
        }
        for (const contract of row.contracts) {
            if (named.some((id) => model.slices.get(id)?.designIds.has(contract)))
                continue;
            findings.push(finding("system_scope_conflict", "amendment", "design.md", `${contract}: Requirement Trace row names slice(s) ${label}, none declaring the contract`, SYSTEM_SCOPE_BINDING_REQUIRED, `declare ${contract} in the named slice's Design IDs or fix the trace row`));
        }
        for (const seit of row.seits) {
            if (named.some((id) => model.slices.get(id)?.proofRowIds.has(seit)))
                continue;
            findings.push(finding("system_scope_conflict", "amendment", "design.md", `${seit}: Requirement Trace row names slice(s) ${label}, none listing the row in SEIT proof rows`, SYSTEM_SCOPE_BINDING_REQUIRED, `add ${seit} to the named slice's SEIT proof rows or fix the trace row`));
        }
        for (const path of new Set(row.paths)) {
            if (!producedPaths.has(path))
                continue;
            if (named.some((id) => model.manifests.get(id)?.writeSetPaths.includes(path)))
                continue;
            findings.push(finding("system_path_conflict", "amendment", "design.md", `${path}: Requirement Trace row names slice(s) ${label}, none covering the path in a write set`, SYSTEM_PATH_BINDING_REQUIRED, `add \`${path}\` to the named slice's write set or fix the trace row`));
        }
    }
    return findings;
}
const MAX_RATIONALE = 4096;
const RISK_COVERAGE_REQUIRED = "every yes flag must map a design section or SYS- system, at least one SEIT row, and a slice; every no flag must carry an evidence-backed not-applicable rationale";
 ...
 */
function evidenceBackedRationale(value) {
    const rationale = value.trim();
    if (!rationale || rationale.length > MAX_RATIONALE)
        return false;
    if (/^(?:-|tbd|todo|n\/a|na|none|no|not\s+applicable)\.?$/i.test(rationale))
        return false;
    if (rationale.split(/\s+/).length < 4)
        return false;
    if (VACUOUS_NEGATION.test(rationale))
        return false;
    if (DEFERRAL.test(rationale))
        return false;
    return true;
}
/**
 ...
 */
function estimateDeclaredTokens(text) {
    return Math.ceil(Buffer.byteLength(text, "utf8") / 4);
}
const SLICE_SCOPE_AIM = `a slice's declared text — its heading section plus its execution manifest — aims to stay within ${SLICE_WORKLOAD_AIM_TOKENS} estimated tokens so one agent is handed bounded work. The aim is ergonomic policy, not a prediction of the effort the slice will consume, and it never changes the verdict`;
function sliceScopeFindings(model) {
    const findings = [];
    for (const [id, slice] of model.slices) {
        const declared = `${slice.raw}\n${model.manifests.get(id)?.raw ?? ""}`;
        const tokens = estimateDeclaredTokens(declared);
        if (tokens <= SLICE_WORKLOAD_AIM_TOKENS)
            continue;
        findings.push(finding("slice_scope_advisory", "advisory", "implementation.md", `${id} declares about ${tokens} estimated tokens across its slice section and execution manifest, above the ${SLICE_WORKLOAD_AIM_TOKENS}-token aim`, SLICE_SCOPE_AIM, "split the slice into smaller slices, or keep it deliberately — the aim does not block the plan", id));
    }
    return findings;
}
function riskFindings(model) {
    const findings = [];
    ...
    for (const [flag, entry] of model.riskProfile) {
        if (entry.coverageIssues.length)
            continue;
        if (entry.applies === "no") {
            if (!evidenceBackedRationale(entry.rationale)) {
                findings.push(finding("risk_coverage_missing", "amendment", "plan-spec.md", `${flag}: not-applicable rationale is not evidence-backed`, "every no flag must carry an evidence-backed not-applicable rationale", "state why the flag does not apply or declare coverage"));
            }
            continue;
        }
        ...
        if (!designClauses.length && !systemClauses.length) {
            findings.push(finding("risk_coverage_missing", "amendment", "plan-spec.md", `${flag}: design coverage missing`, RISK_COVERAGE_REQUIRED, "map the flag to a design section or SYS- system, a SEIT row, and a slice"));
        }
        for (const clause of designClauses) {
            if (!sectionPresent(model.documents.design, clause.value)) {
                findings.push(finding("risk_coverage_missing", "amendment", "plan-spec.md", `${flag}: design section ${clause.value} is not a non-empty design section`, RISK_COVERAGE_REQUIRED, `map the flag to a non-empty ## section in design.md`));
            }
        }
        for (const clause of systemClauses) {
            if (!model.systemCatalog.has(clause.value)) {
                findings.push(finding("risk_coverage_missing", "amendment", "plan-spec.md", `${flag}: system ${clause.value} is not a System Catalog entry`, RISK_COVERAGE_REQUIRED, `add ${clause.value} to the System Catalog or name a design section`));
            }
        }
        if (!seitClauses.length) {
            findings.push(finding("risk_coverage_missing", "amendment", "plan-spec.md", `${flag}: SEIT coverage missing`, RISK_COVERAGE_REQUIRED, "map the flag to at least one declared SEIT row"));
        }
        for (const clause of seitClauses) {
            if (!model.traceRows.has(clause.value)) {
                findings.push(finding("risk_coverage_missing", "amendment", "plan-spec.md", `${flag}: SEIT row ${clause.value} is not a declared traceability row`, RISK_COVERAGE_REQUIRED, `map the flag to a declared SEIT- traceability row`));
            }
        }
        if (!sliceClauses.length) {
            findings.push(finding("risk_coverage_missing", "amendment", "plan-spec.md", `${flag}: slice coverage missing`, RISK_COVERAGE_REQUIRED, "map the flag to at least one declared slice"));
        }
        for (const clause of sliceClauses) {
            if (!model.slices.has(clause.value)) {
                findings.push(finding("risk_coverage_missing", "amendment", "plan-spec.md", `${flag}: slice ${clause.value} is not a declared slice`, RISK_COVERAGE_REQUIRED, `map the flag to a declared slice id`));
            }
        }
    }
    return findings;
}
const SHARED_CONTRACT_PRODUCER_REQUIRED = "every declared shared interface path must be covered by a slice write set so the route can produce the shared contract";
function sharedContractFindings(model) {
    const findings = [];
    ...
    for (const [id, manifest] of model.manifests) {
        for (const identifier of manifest.sharedInterfaces ?? []) {
            ...
            if (!path || (!identifier.includes("#") && !path.includes("/")))
                continue;
            if (producedPaths.has(path))
                continue;
            findings.push(finding("shared_contract_unproduced", "amendment", "implementation.md", path, SHARED_CONTRACT_PRODUCER_REQUIRED, `add \`${path}\` to a slice write set or remove the shared-interface declaration`, id));
        }
    }
    return findings;
}
...
function focusEnvelope(model, sliceId) {
    const closure = [];
    const seen = new Set();
    const queue = [sliceId];
    while (queue.length) {
...
function focusEnvelopePaths(model, sliceId) {
    const paths = new Set();
 ...
 */
function focusEnvelopePathFindings(model, findings) {
    const aggregateIndex = findings.findIndex((item) => item.code === "writeset_unsafe_path" && item.sliceId === undefined && /^\d+ unique paths$/.test(item.observed));
    if (aggregateIndex < 0)
        return [...findings];
    ...
    for (const [id, paths] of [...model.slices.keys()].map((id) => [id, focusEnvelopePaths(model, id)])) {
        if (paths.size <= MAX_FOCUS_ENVELOPE_PATHS)
            continue;
        result.push(finding("writeset_unsafe_path", "amendment", "implementation.md", `${paths.size} unique paths`, `the Focus envelope for Slice ${id} (its write set plus prerequisite closure) may aggregate at most ${MAX_FOCUS_ENVELOPE_PATHS} paths`, "split the slice's write set or restructure its dependencies", id));
    }
    return result;
}
...
export function validatePlan(input) {
    ...
    const findings = focusEnvelopePathFindings(model, [...structuralFindings(model), ...semanticFindings(model)]);
