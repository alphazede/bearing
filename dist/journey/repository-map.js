...
async function artifactSafe(root, artifact, allowMissing = false) {
    try {
...
export async function inventory(root) {
    ...
    const visit = async (directory, prefix, depth) => {
        if (depth > MAX_DEPTH || paths.length >= MAX_PATHS)
            return;
...
function mapText(repository, paths) {
    return [
        "---",
        "type: repository-map",
...
export async function setBearingsWorkspace(repository, goal, existingDirectory) {
    if (!existingDirectory || !planDirectoryValid(existingDirectory))
        return undefined;
    const plans = resolve(repository, "docs/plans");
    await mkdir(plans, { recursive: true });
    if (!inside(repository, await realpath(plans)))
        return undefined;
    let directory = plans;
    let resumed = true;
    ...
    if (!canonicalDirectory || !await artifactSafe(canonicalDirectory, planSpec, true))
        return undefined;
    ...
    if (!canonicalDirectory || !await containedDirectory(canonicalDirectory, prompts))
        return undefined;
    const map = resolve(prompts, "repository-map.md");
    if (!await artifactSafe(canonicalDirectory, map, true))
        return undefined;
    try {
    ...
    try {
        await writeFile(map, mapText(repository, await inventory(repository)), { flag: "wx" });
    }
    catch (error) {
        if (!(error instanceof Error) || !("code" in error) || error.code !== "EEXIST")
            throw error;
    }
    if (!await artifactSafe(canonicalDirectory, planSpec) || !await artifactSafe(canonicalDirectory, map))
        return undefined;
