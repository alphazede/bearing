...
async function planInventory(root, { maximumDepth = MAX_DEPTH, strictDepth = true } = {}) {
    const paths = [];
    let complete = true;
...
async function resolveFullPath(repository, path) {
    if (!planDirectoryValid(path))
...
export async function resolvePlanDirectory(repository, request) {
    if (request.startsWith("docs/plans/"))
        return resolveFullPath(repository, request);
    ...
    if (!canonicalPlans) {
        try {
            ...
            if (missing(error))
                return { ok: false, reason: "plan_directory_absent", requested: request };
            throw error;
        }
        return { ok: false, reason: "plan_directory_invalid" };
    }
    ...
    if (matches.length === 0)
        return { ok: false, reason: "plan_directory_absent", requested: request };
    if (matches.length > 1)
        return { ok: false, reason: "plan_directory_ambiguous", matches };
    return resolveFullPath(repository, matches[0]);
}
async function requiredDirectory(repository, path) {
    if (!planDirectoryValid(path))
...
async function directoryState(repository, root, path) {
    let directory = root;
    let absent = false;
...
async function parentState(repository, root, path) {
    return directoryState(repository, root, dirname(path));
}
export async function planConsolidation(repository, canonical, sources) {
    ...
    for (const source of sources) {
        ...
        if (sourceDirectory === canonicalDirectory)
            throw new Error("plan_directory_invalid");
        const inventory = await planInventory(sourceDirectory);
        if (!inventory.complete)
            return { ok: false, reason: "consolidation_conflict", canonical, sources, entries };
...
async function ensureParent(repository, canonical, path) {
...
export async function applyConsolidation(repository, plan) {
    if (!plan.ok)
        return { ok: false, reason: plan.reason, sources: plan.sources };
