...
export function planWorkspaceName(planDirectory) {
...
export function planWorkspacePath(repositoryRoot, planDirectory) {
...
export function isVisibleWorkspaceName(name) {
 ...
 * a Bearing workspace.
 */
export async function visibleWorkspaces(repositoryRoot) {
    const names = [];
    let entries;
    try {
    ...
    for (const name of entries) {
        if (!isVisibleWorkspaceName(name))
            continue;
...
export function workspaceRelativePath(repositoryRoot, workspacePath) {
    if (!workspacePath.startsWith(repositoryRoot))
        return undefined;
    const rest = workspacePath.slice(repositoryRoot.length).replace(/^[\\/]+/, "");
    if (!rest || rest.includes("\\") || posix.normalize(rest) !== rest)
        return undefined;
    return rest;
}
