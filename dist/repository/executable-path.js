...
function isValidBuildIdentifier(s) {
...
function parseSemver(version) {
    ...
    if (preRaw !== undefined) {
        if (preRaw.length === 0 || preRaw.startsWith(".") || preRaw.endsWith("."))
            return undefined;
        const parts = preRaw.split(".");
        if (parts.some((p) => p.length === 0))
            return undefined;
        for (const p of parts) {
            if (!isValidPrereleaseIdentifier(p))
                return undefined;
        }
        prerelease = parts;
    }
    if (buildRaw !== undefined) {
        if (buildRaw.length === 0 || buildRaw.startsWith(".") || buildRaw.endsWith("."))
            return undefined;
        const parts = buildRaw.split(".");
        if (parts.some((p) => p.length === 0 || !isValidBuildIdentifier(p)))
            return undefined;
...
function compareSemver(a, b) {
    ...
    if (aPre === undefined && bPre === undefined)
        return 0;
    if (aPre === undefined)
        return 1; // stable outranks prerelease
    if (bPre === undefined)
        return -1;
...
export function resolveBearingCli(deps = {}) {
    ...
    const pathCandidate = { path: pathCliPath, version: identity.version };
