...
const rows = (values) => values.slice(0, MAX_ROWS);
export function renderLaunchReadinessSvg(claims) {
...
export class ReportRenderer {
    render(ledger, title = "Bearing evidence report") {
        if (typeof title !== "string" || title.length === 0 || title.length > 128 || /[\x00-\x1f\x7f]/.test(title))
            throw new Error("invalid_report_title");
        assertPublicLedger(ledger);
        const svg = renderLaunchReadinessSvg(ledger.claims);
