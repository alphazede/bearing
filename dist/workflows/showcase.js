...
import { fictionalB2bWorkflows } from "./catalog.js";
const MAX_WORKFLOW_ID = 64;
export const MAX_SHOWCASE_JSON = 640 * 1024;
export const MAX_SHOWCASE_REPORT = 128 * 1024;
const WORKFLOW_ID = /^[A-Za-z0-9][A-Za-z0-9.-]{0,63}$/;
...
const ledgerFor = (id) => {
    if (id === "workflow.engineering-import.v1")
        return engineeringLedger();
    if (id === "workflow.launch-readiness.v1")
        return launchLedger();
    if (id === "workflow.due-diligence.v1")
        return dueDiligenceLedger();
    throw new Error("unknown_workflow");
};
const publicDefinition = (workflow) => frozenCopy(structuredClone({
    id: workflow.id,
    name: workflow.name,
...
export function projectWorkflowShowcase(id) {
    if (!WORKFLOW_ID.test(id) || id.length > MAX_WORKFLOW_ID)
        return null;
...
export function renderWorkflowReport(id) {
