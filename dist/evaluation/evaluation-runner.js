...
function text(value, maximum = 128) { return typeof value === "string" && value.length > 0 && value.length <= maximum; }
function unique(values) { return new Set(values).size === values.length; }
function average(values) { return values.reduce((sum, value) => sum + value, 0) / values.length; }
function compositeKey(suiteId, caseId, arm, route, trial) { return JSON.stringify([suiteId, caseId, arm, route, trial]); }
function keyOf(cell) { return compositeKey(cell.suiteId, cell.caseId, cell.arm, cell.route, cell.trial); }
function expectedKey(suite, caseId, arm, route, trial) { return compositeKey(suite.suiteId, caseId, arm, route, trial); }
function validSuite(value) {
    if (!exact(value, ["schemaVersion", "suiteId", "kind", "version", "caseIds", "arms"]))
        return false;
    if (value.schemaVersion !== EVALUATION_SCHEMA_VERSION || !text(value.suiteId) || !["native", "skillsbench"].includes(value.kind) || !text(value.version) || !Array.isArray(value.caseIds) || value.caseIds.length === 0 || value.caseIds.length > MAX_CASES || !value.caseIds.every((id) => text(id)) || !unique(value.caseIds))
        return false;
    if (!exact(value.arms, ["control", "treatment"]) || !text(value.arms.control) || !text(value.arms.treatment) || value.arms.control === value.arms.treatment)
        return false;
    if (value.kind === "native")
        return value.arms.control === "without-skill" && value.arms.treatment === "with-skill";
...
function validCell(value) {
    ...
    if (!object(value) || !exact(value, value.outcome === "failed" ? [...required, "failure"] : required))
        return false;
    if (![value.suiteId, value.caseId, value.arm, value.route, value.workspaceId].every((entry) => text(entry)) || !TRIALS.includes(value.trial))
        return false;
    if (!exact(value.identity, ["requested", "effective"]) || !text(value.identity.requested) || !text(value.identity.effective))
        return false;
    if (!exact(value.metadata, ["source", "provider", "model", "reasoning", "harness", "isolation"]) || !["verified-provider", "synthetic"].includes(value.metadata.source) || ![value.metadata.provider, value.metadata.model, value.metadata.harness].every((entry) => text(entry)) || !["low", "medium", "high", "xhigh"].includes(value.metadata.reasoning) || !["attested", "local"].includes(value.metadata.isolation) || !((value.metadata.source === "verified-provider" && value.metadata.isolation === "attested") || (value.metadata.source === "synthetic" && value.metadata.isolation === "local")))
        return false;
    if (!exact(value.failure, ["code", "message"]) && value.outcome === "failed")
        return false;
    if (value.outcome === "failed" && (!text(value.failure.code) || !text(value.failure.message, 4096)))
        return false;
    return ["passed", "failed"].includes(value.outcome) && typeof value.criticalInvariantPassed === "boolean" && typeof value.score === "number" && Number.isFinite(value.score) && value.score >= 0 && value.score <= 1;
}
...
export class EvaluationRunner {
    runSuite(definition, results) {
        if (!validSuite(definition))
            throw new TypeError("invalid evaluation suite definition");
        const suite = definition;
        const cells = new Map();
        const issues = [];
        const routeIssues = new Map(EVALUATION_ROUTES.map((route) => [route, []]));
        const failures = [];
        ...
        if (!Array.isArray(results) || results.length > MAX_RESULTS)
            issues.push("results:malformed");
        values.forEach((value, index) => {
            if (!validCell(value)) {
                const issue = `cell:${index}:malformed`;
                issues.push(issue);
                if (object(value) && typeof value.route === "string" && routeIssues.has(value.route))
                    routeIssues.get(value.route).push(issue);
                return;
            }
            const key = keyOf(value);
            if (value.outcome === "failed")
                failures.push({ key, ...value.failure });
            if (cells.has(key)) {
        ...
        for (const cell of cells.values()) {
            ...
            if (expectedMetadata === undefined)
                routeMetadata.set(cell.route, metadata);
        ...
        const routes = EVALUATION_ROUTES.map((route) => {
            ...
            const verdict = !complete ? "incomplete" : uplift > 0 && criticalInvariantsPassed && noCaseRegression && outcomesPassed ? "passed" : "failed";
        ...
        const verdict = issues.length > 0 || routes.some(({ verdict }) => verdict === "incomplete") ? "incomplete" : allRoutesPassed ? "passed" : "failed";
        return deepFreeze({
    ...
    verdict(definition, results) { return this.runSuite(definition, results); }
}
