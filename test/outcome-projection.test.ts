...
const FIRST_COMMAND_HASH = "d".repeat(64);
const SECOND_COMMAND_HASH = "e".repeat(64);
...
/** Checkpoint carrying one recovery outcome under an explicit accepted-command identity. */
function recoveryCheckpoint(
  recoveryOutcome: Readonly<Record<string, unknown>>,
  sequence: number,
  commandContentHash: string,
): EventEnvelopeV1 {
  return { ...checkpoint({ recoveryOutcome }, sequence), commandContentHash };
}
...
describe("outcome projection", () => {
  ...
  it("emits each bounded cumulative token total once as a deterministic delta", () => {
    ...
    expect(tokenRecords.map(({ tokens, budget }) => ({ tokens, budget }))).toEqual([
      { tokens: 0, budget: 10 },
      { tokens: 3, budget: 10 },
      { tokens: 2, budget: 10 },
    ]);
  ...
  it("retains per-invocation budget state while projecting cumulative totals as deltas", () => {
    ...
    expect(tokenRecords.map(({ code, tokens, budget }) => ({ code, tokens, budget }))).toEqual([
      { code: "within_budget", tokens: 90_000, budget: 200_000 },
      { code: "within_budget", tokens: 90_000, budget: 200_000 },
      { code: "within_budget", tokens: 90_000, budget: 200_000 },
    ]);
  });
  ...
  it("emits both adjacent distinct recovery episodes that share an outcome and attempt count", () => {
    const recoveryOutcome = { outcome: "repaired", attempts: 1 } as const;
    const records = projectOutcomes({
      runId: RUN_ID,
      digest: digestFor(),
      events: [
        recoveryCheckpoint(recoveryOutcome, 1, FIRST_COMMAND_HASH),
        recoveryCheckpoint(recoveryOutcome, 2, SECOND_COMMAND_HASH),
      ],
    });

    expect(records.filter(({ signal }) => signal === "recovery")).toEqual([
      expect.objectContaining({ code: "repaired", attempts: 1 }),
      expect.objectContaining({ code: "repaired", attempts: 1 }),
    ]);
  });

  it("coalesces a replayed recovery checkpoint that carries one durable command identity", () => {
    const recoveryOutcome = { outcome: "repaired", attempts: 1 } as const;
    const records = projectOutcomes({
      runId: RUN_ID,
      digest: digestFor(),
      events: [
        recoveryCheckpoint(recoveryOutcome, 1, FIRST_COMMAND_HASH),
        recoveryCheckpoint(recoveryOutcome, 2, FIRST_COMMAND_HASH),
      ],
    });

    expect(records.filter(({ signal }) => signal === "recovery")).toHaveLength(1);
  });

  it("fails closed to value coalescing when adjacent recovery command identity is absent or unreadable", () => {
    const recoveryOutcome = { outcome: "repaired", attempts: 1 } as const;
    for (const [first, second] of [
      ["not-a-digest", "not-a-digest"],
      ["", SECOND_COMMAND_HASH],
      [FIRST_COMMAND_HASH, ""],
      ["not-a-digest", SECOND_COMMAND_HASH],
    ] as const) {
      const records = projectOutcomes({
        runId: RUN_ID,
        digest: digestFor(),
        events: [
          recoveryCheckpoint(recoveryOutcome, 1, first),
          recoveryCheckpoint(recoveryOutcome, 2, second),
        ],
      });
