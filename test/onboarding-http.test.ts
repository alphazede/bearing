...
describe("repository-first onboarding HTTP", () => {
  ...
  it("persists the verified route and drives the real journey through contained HTML evidence", async () => {
    ...
    expect(JSON.parse((await journey("gather-supplies")).body)).toMatchObject({ status: "question", question: "Which acceptance risk matters most?", recovery: { status: "repaired", stage: "gather-supplies", failureClass: "agent_receipt_or_artifact_validation", code: "result_missing", retryLevel: "repair", version: "0.1.7" } });
