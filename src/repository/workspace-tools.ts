...
export interface WorkspaceToolsDeps {
  ...
  readonly storeFactory?: (repository: string) => RetentionStore;
}
...
export interface WorkspaceRetentionOptions {
  ...
  readonly onPlan?: (lines: readonly string[]) => void;
}
...
export async function workspaceCompact(
...
export async function workspacePrune(
  options: WorkspaceRetentionOptions,
  deps: WorkspaceToolsDeps = {},
): Promise<DoctorResult> {
...
export async function workspaceDoctor(
  ...
  for (const candidate of candidates) {
    if (await isDirectory(candidate)) detected.push(candidate);
  }
 ...
 */
export async function writeWorkspaceBusyLease(
  repository: string,
...
async function safeWorkspaceDirectory(repository: string, create: true): Promise<string>;
async function safeWorkspaceDirectory(repository: string, create: false): Promise<string | undefined>;
async function safeWorkspaceDirectory(repository: string, create: boolean): Promise<string | undefined> {
  const workspace = join(repository, BEARING_DIR);
  if (create) await mkdir(workspace, { recursive: true, mode: 0o700 });
  let entry;
  try {
...
async function runBreakdown(repository: string): Promise<readonly string[]> {
  const runsRoot = join(repository, BEARING_DIR, "runs");
  let entries;
  try {
    entries = await readdir(runsRoot, { withFileTypes: true });
  } catch (error) {
    if (isNodeError(error, "ENOENT")) return ["Runs: 0 (settled: 0, unsettled: 0, compacted: 0)"];
    throw error;
  }
  ...
  let unsettled = 0;
  let compacted = 0;
  const unreadableRuns: Array<{ readonly runId: string; readonly integrityError: string }> = [];
  for (const entry of states) {
    ...
    const { state } = entry;
    if (isCompactedRunState(state)) compacted += 1;
    else if (state.pendingDecision === null &&
      state.journeyCheckpoint?.stage === "review" &&
      state.journeyCheckpoint.status === "complete") settled += 1;
    else unsettled += 1;
  }
...
async function liveCallerCleanlinessProof(
  repository: string,
  deps: WorkspaceToolsDeps,
): Promise<CallerCleanlinessProof | undefined> {
  const git = deps.git ?? defaultGit;
  const listed = await git(repository, ["worktree", "list", "--porcelain", "-z"]);
  if (listed.exitCode !== 0) throw new Error("could not determine the Git worktree set");
  ...
  for (const worktree of canonicalWorktrees) {
    ...
    if (status.exitCode !== 0) throw new Error(`could not determine worktree cleanliness: ${worktree.path}`);
    ...
    if (merged.exitCode !== 0) throw new Error(`could not determine whether a worktree is merged: ${worktree.path}`);
  }

  const lease = await busyLeaseState(repository, deps.now?.() ?? new Date());
  if (lease === "busy") return undefined;
  return { noDirtyOrUnmergedLane: true, runNotBusy: true };
}

function parseWorktrees(output: string): readonly WorktreeRecord[] {
  const fields = output.split("\0");
  if (fields.at(-1) !== "") throw new Error("could not determine the Git worktree set");
  ...
  if (records.length === 0) throw new Error("could not determine the Git worktree set");
...
async function busyLeaseState(repository: string, now: Date): Promise<"idle" | "busy"> {
  ...
  try {
    ...
    if (!isBusyLease(parsed)) throw new Error("busy lease is unreadable or ambiguous");
    ...
    if (isNodeError(error, "ENOENT")) return "idle";
    if (error instanceof Error && error.message === "busy lease is unreadable or ambiguous") throw error;
...
function isBusyLease(value: unknown): value is {
  ...
  if (Object.keys(record).sort().join(",") !== "expiresAt,runIds,schemaVersion" ||
    ...
    typeof record.expiresAt !== "string") return false;
  const expires = Date.parse(record.expiresAt);
  return Number.isFinite(expires) && new Date(expires).toISOString() === record.expiresAt;
}

const defaultGit: GitRunner = (cwd, args) => new Promise((resolveResult, reject) => {
  execFile("git", [...args], {
    cwd,
    encoding: "utf8",
    timeout: 10_000,
    ...
    if (typeof code === "number") resolveResult({ exitCode: code, stdout });
    else reject(error);
  });
});

async function discoverGitRoot(start: string): Promise<string | undefined> {
...
async function directoryRealpath(path: string): Promise<string> {
  ...
  if (!await isDirectory(resolved)) throw new Error(`not a directory: ${path}`);
  return resolved;
}
...
async function ignoresBearing(repository: string): Promise<boolean> {
  const path = join(repository, ".gitignore");
  if (!await lstat(path).then((entry) => entry.isFile() && !entry.isSymbolicLink()).catch(() => false)) return false;
  return ignoresBearingDirectory(await readFile(path, "utf8"));
}

async function knownAgentRealpaths(pathEnv: string): Promise<readonly string[]> {
  const found: string[] = [];
