...
import { RepositoryBootstrap, gitignoreCoversBearingState, ignoresBearingDirectory, ignoresBearingWorkspaces } from "../repository/bootstrap.js";
...
import { JourneyService, currentPlanningVerdict, expeditionSliceDependencyRefusal, planningCheckpointFields, reconOwnerDecisionQuestion, type JourneyActivity, type JourneyReconResult, type JourneyResult, type JourneySelectedScope, type JourneyStage } from "../journey/planning-journey.js";
import {
  canonicalStringify,
  isJourneyTokenUsage,
  ...
  type VerificationConvergenceProjection,
...
import { EMPTY_CONVERGENCE_HISTORY, findingIdentity, parseParkRangerReport, recordReviewCycle, synthesizeFindings, type ConvergenceHistory, type ParkRangerFinding, type ParkRangerReportFailure } from "../verification/park-ranger.js";
...
type ImprovementHandoffNextAction = (typeof IMPROVEMENT_HANDOFF_NEXT_ACTIONS)[number];
...
type RuntimeImprovementReport = ImprovementReport<Thresholds, MetricSet, RecommendationResult> & {
...
function writeJourneyFailure(res: ServerResponse, status: number, code: "input_invalid" | "fit_undecidable" | "dependency_refused"): void {
...
async function handleRepositoryGitignorePost(
  ...
  try {
    ...
    if (!ignoresBearingDirectory(body)) additions.push(".bearing/");
    ...
    if (!ignoresBearingWorkspaces(body)) additions.push("/bearing-*/");
 ...
 */
const PLANNING_DOCUMENT_STAGES = new Set<JourneyStage>(["map-route", "draft-implementation"]);
 ...
 */
async function planContentHashFor(repositoryPath: string, planDirectory: string | null | undefined): Promise<string | null> {
  if (typeof planDirectory !== "string" || !planDirectoryValid(planDirectory)) return null;
  let names: string[];
  try { names = await readdir(join(repositoryPath, planDirectory)); }
  catch { return null; }
  const hash = createHash("sha256");
  for (const name of names.filter((entry) => entry !== ".bearing").sort()) {
    ...
    try {
      if (!(await lstat(entryPath)).isFile()) continue;
    } catch { return null; }
    let content: Buffer;
    try { content = await readFile(entryPath); }
    catch { return null; }
 ...
 * execution contract).
 */
function completedSliceIdsFromDurable(durable: Awaited<ReturnType<BearingStore["load"]>>): ReadonlySet<string> {
  ...
  for (const event of durable.events) {
    ...
    if (parsed.ok && parsed.value.layer === "validator" && parsed.value.verdict === "PASS") {
      for (const slice of parsed.value.completedSlices ?? []) completed.add(slice.sliceId);
    }
    const lastResult = parseCheckpointJson<JourneyResult | undefined>(event.payload.lastResultJson, undefined);
    if (lastResult?.status === "action" && lastResult.verification?.verdict === "PASS") {
      for (const slice of lastResult.verification.completedSlices ?? []) completed.add(slice.sliceId);
    }
  }
  return completed;
}
...
function handleJourneyPost(
  ...
  readJsonBody(req, MAX_JOURNEY_BODY).then(async (value) => {
    ...
    if (value.stage === "execute-expedition" && value.currentSlice && state.planDirectory) {
      ...
      const refusal = await expeditionSliceDependencyRefusal(
        repositoryPath,
        state.planDirectory,
        value.currentSlice,
        completedSliceIdsFromDurable(durable),
      );
      if (refusal) {
    ...
    let inspection: HeadlessRunInspection | undefined;
    try {
...
async function handleImprovementReportGet(
  ...
  const selectedReport = report ?? ((input: { readonly repositoryPath: string; readonly store: BearingStore }) => (
    buildImprovementReport(input.store, input.repositoryPath)
  ));
...
interface RoleRouteView {
  readonly primary: string;
  readonly fallbacks: readonly string[];
}

interface GuidedRoleRoutes {
...
type RoleRoutesRequiredOwnerAction = {
 ...
 */
function convergenceHistoryFromEvents(events: Awaited<ReturnType<BearingStore["load"]>>["events"], excludeCommandId: string): ConvergenceHistory {
  let history = EMPTY_CONVERGENCE_HISTORY;
  for (const event of events) {
    if (event.type !== "journeyCheckpointRecorded" || event.causationId === excludeCommandId) continue;
    const parsed = parseVerificationCheckpoint(event.payload.verification);
    if (!parsed.ok || parsed.value.layer !== "park-ranger" || parsed.value.convergence === undefined) continue;
    history = {
...
async function handleVerificationReportPost(
  ...
  let synthesizedFindings: readonly ParkRangerFinding[] | undefined;
  ...
  if (synthesizedFindings !== undefined) {
    ...
    if (!cycle.ok) { writeRunReadFailure(res, "verification_projection_invalid"); return; }
    verification = {
      ...verification,
      convergence: {
...
/**
 * One bounded event of a run's audit trail, projected for inspection surfaces:
 * sequence, ledger timestamp, and type only. Event payloads are never projected
 * — they can carry owner-authored text and provider detail that must not flow
 * beyond the already-trusted loopback and CLI boundaries.
 */
interface HeadlessInspectedEvent {
  readonly sequence: number;
  readonly recordedAt: string;
...
/**
 * Validation outcome of the current run, projected for inspection surfaces.
 * Each recorded part is projected independently: the planning-validation
 * verdict (PASS / NEEDS_AMENDMENT / OWNER_DECISION_REQUIRED) and the recorded
 * verification checkpoint (layer and verdict) can exist without each other.
 */
interface HeadlessValidationProjection {
  readonly planVerdict?: PlanningValidationRecord["verdict"];
  readonly verification?: { readonly layer: VerificationLayer; readonly verdict: VerificationVerdict };
}

/**
 * Where the current run's durable state lives and what the inspection surfaces
 * (CLI receipt, browser journey panel) disclose about it. `stateLocation` is a
 * repository-relative path in the visible `bearing-<plan>/` workspace when the
 * run is plan-bound, or the legacy `.bearing/runs/<runId>` home otherwise.
 */
interface HeadlessRunInspection {
  readonly stateLocation: string;
  /** Visible per-plan workspace name once the run is plan-bound; absent for legacy `.bearing` runs. */
  readonly workspace?: string;
  readonly events: readonly HeadlessInspectedEvent[];
  readonly allowedActions: readonly string[];
  /** Focus scope of the latest recorded result, when the run carries one. */
  readonly focusScope?: JourneySelectedScope;
  readonly validation?: HeadlessValidationProjection;
}
...
export type HeadlessJourneyReceipt = {
  ...
  /** Repository-relative audit-trail path for the current run, `workspace/runs/<runId>` for plan-bound runs and `.bearing/runs/<runId>` for legacy ones (issue 56). */
  readonly stateLocation?: string;
  /** Most recent bounded slice of the run's recorded events, newest-last, in ledger sequence order (issue 56). */
  readonly events?: readonly HeadlessInspectedEvent[];
  /** Bounded projection of the latest planning validation outcome and verification verdict, present only when one was recorded (issue 56). */
  readonly validation?: HeadlessValidationProjection;
};
...
const HEADLESS_INSPECTED_EVENT_TYPES: readonly EventType[] = [
  "workRequestCreated",
  "decisionRequired",
  "ownerAnswered",
...
const HEADLESS_PLAN_VERDICTS: readonly string[] = ["PASS", "NEEDS_AMENDMENT", "OWNER_DECISION_REQUIRED"];
const HEADLESS_VERIFICATION_LAYERS: readonly string[] = ["validator", "grader", "park-ranger"];
 ...
 */
async function headlessRunInspection(
  store: BearingStore,
  runId: string,
  durable: StoredRunState,
  state: BrowserJourney,
  flags: { readonly planReviewAvailable: boolean; readonly explorerAvailable: boolean; readonly recoveryAvailable: boolean },
): Promise<HeadlessRunInspection> {
  ...
  const allowedActions = [
    "status",
    ...(flags.planReviewAvailable ? ["approve-route" as const] : []),
    ...(flags.explorerAvailable ? ["select-explorer" as const] : []),
    ...(flags.recoveryAvailable ? ["resume" as const] : []),
  ];
  ...
  const validation: HeadlessValidationProjection | undefined = planningValidation || verification
    ? {
...
function headlessStateLocation(value: unknown): string | undefined {
 ...
 */
function headlessInspectedEvents(value: unknown): readonly HeadlessInspectedEvent[] | undefined {
  if (!Array.isArray(value) || value.length === 0 || value.length > MAX_INSPECTED_EVENTS) return undefined;
  const projected: HeadlessInspectedEvent[] = [];
  for (const candidate of value) {
    if (typeof candidate !== "object" || candidate === null) return undefined;
    const event = candidate as Record<string, unknown>;
    if (!Number.isSafeInteger(event.sequence) || (event.sequence as number) < 0) return undefined;
    if (typeof event.recordedAt !== "string" || !HEADLESS_EVENT_TIMESTAMP.test(event.recordedAt)) return undefined;
    if (typeof event.type !== "string" || !HEADLESS_INSPECTED_EVENT_TYPES.includes(event.type as EventType)) return undefined;
    projected.push({ sequence: event.sequence as number, recordedAt: event.recordedAt, type: event.type as EventType });
  }
  return projected;
}

/** Bound a validation projection read from JSON; the whole field fails closed on any unknown value. */
function headlessInspectionValidation(value: unknown): HeadlessValidationProjection | undefined {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return undefined;
  const validation = value as Record<string, unknown>;
  const planVerdict = validation.planVerdict;
  if (planVerdict !== undefined && (typeof planVerdict !== "string" || !HEADLESS_PLAN_VERDICTS.includes(planVerdict))) return undefined;
  const verification = validation.verification;
  if (verification === undefined) {
    return typeof planVerdict === "string" ? { planVerdict: planVerdict as PlanningValidationRecord["verdict"] } : undefined;
  }
  if (typeof verification !== "object" || verification === null || Array.isArray(verification)) return undefined;
  const layer = (verification as Record<string, unknown>).layer;
  const verdict = (verification as Record<string, unknown>).verdict;
  if (typeof layer !== "string" || !HEADLESS_VERIFICATION_LAYERS.includes(layer)) return undefined;
  if (!isVerificationVerdict(layer as VerificationLayer, verdict)) return undefined;
  return {
    ...(typeof planVerdict === "string" ? { planVerdict: planVerdict as PlanningValidationRecord["verdict"] } : {}),
    verification: { layer: layer as VerificationLayer, verdict: verdict as VerificationVerdict },
  };
}
...
type DurableContinuationProjection = ReturnType<typeof projectHeadlessReceipt>;
...
export type DurableRunContinuation =
  ...
  | {
      ...
      /** Repository-relative visible per-plan workspace directory of the run's audit trail, once plan-bound. */
      readonly workspace?: string;
      /** Repository-relative path of the run's audit trail (`<workspace>/runs/<runId>`), once plan-bound. */
...
type LegacyExecutionContractFailure =
  | "repository_rejected"
  | "store_unreadable"
  | "run_not_found"
