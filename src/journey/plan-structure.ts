...
type KnownRiskFlag = (typeof KNOWN_RISK_FLAGS)[number];
const RISK_PROFILE_HEADING = /^##[ \t]+Risk Profile[ \t]*$/mi;
const RISK_PROFILE_COLUMNS = ["flag", "applies", "coverage or rationale"] as const;
...
const SYSTEM_CATALOG_COLUMNS = ["system id", "system", "responsibility"] as const;
const SYSTEM_TRACE_COLUMNS = ["requirement id", "system id", "contract id", "seit row id", "slice id", "path"] as const;
...
export const requiredManifestFields = ["Write set", "Command IDs", "Stop condition", "Human decision"] as const;
const optionalManifestFields = ["Shared interfaces", "Integration boundary", "Parallel safe"] as const;
...
export type PlanArtifact = "plan-spec.md" | "design.md" | "seit.md" | "implementation.md";
...
export type StructuralFindingCode =
  ...
  | "writeset_multiline"
  | "writeset_missing_only"
  | "goal_unbounded"
  | "writeset_empty"
  | "seit_section_missing"
  | "design_section_missing"
  | "artifact_frontmatter_invalid"
  | "id_format_invalid"
  | "id_unknown"
  | "trace_header_invalid"
  | "trace_cell_placeholder"
  | "slice_manifest_mismatch"
  | "slice_reference_dangling"
  | "wave_noncontiguous"
  | "command_undeclared"
  | "build_command_in_manifest"
  | "optional_manifest_malformed"
  | "parallel_safety_conflict"
  | "phase_graph_malformed"
  | "input_unproduced"
  | "system_map_malformed"
  | "risk_profile_malformed";

export interface StructuralFinding {
  readonly code: StructuralFindingCode;
  readonly severity: "advisory" | "amendment";
...
export interface PathIssue {
  readonly code: "writeset_glob" | "writeset_unsafe_path";
  readonly reason: string;
}

export interface WriteSetClauseRejection {
  readonly clause: string;
  readonly paths: readonly string[];
  readonly reason: "ambiguous write authority fails closed rather than silently granting write access; restate the prohibition in prose without backticks per planning contract rule 5";
}
...
interface PlanSlice {
  ...
  readonly designIds: ReadonlySet<string>;
  readonly proofRowIds: ReadonlySet<string>;
}
...
interface PlanManifest {
  ...
  readonly raw: string;
  readonly fields: ReadonlyMap<string, string>;
  readonly writeSetPaths: readonly string[];
  readonly prohibitedWriteSetPaths: readonly string[];
  readonly writeSetClauseRejection?: WriteSetClauseRejection;
...
interface SystemTraceRow {
  readonly requirements: ReadonlySet<string>;
  readonly systems: ReadonlySet<string>;
  readonly contracts: ReadonlySet<string>;
  readonly seits: ReadonlySet<string>;
  readonly slices: ReadonlySet<string>;
  readonly paths: readonly string[];
}

type RiskCoverageClauseKind = "design" | "system" | "seit" | "slice";

interface RiskCoverageClause {
  readonly kind: RiskCoverageClauseKind;
  readonly value: string;
}

interface RiskProfileEntry {
  readonly applies: "yes" | "no";
  readonly clauses: readonly RiskCoverageClause[];
...
export interface PlanModel {
  ...
  readonly slices: ReadonlyMap<string, PlanSlice>;
  readonly manifests: ReadonlyMap<string, PlanManifest>;
  readonly duplicateSliceIds: ReadonlySet<string>;
  readonly duplicateManifestIds: ReadonlySet<string>;
  readonly malformedRecordIds: readonly string[];
  readonly traceHeaders: readonly string[];
  readonly traceRows: ReadonlyMap<string, TraceabilityRow>;
  readonly traceIssues: readonly { readonly observed: string; readonly reason: string }[];
  readonly identifierValues: readonly string[];
  readonly requiredCommands: ReadonlySet<string>;
  readonly planIds: ReadonlySet<string>;
  readonly designIds: ReadonlySet<string>;
  readonly waves: ReadonlyMap<number, ReadonlySet<string>>;
  readonly dependencies: ReadonlyMap<string, ReadonlySet<string>>;
  readonly phaseGraph: readonly PlanPhaseGraphEntry[];
  readonly phaseGraphIssues: readonly string[];
  readonly procedureNarratives: ReadonlyMap<string, readonly ProcedureNarrative[]>;
  readonly systemCatalogAdopted: boolean;
  readonly systemCatalog: ReadonlyMap<string, SystemCatalogEntry>;
  readonly systemCatalogIssues: readonly string[];
  readonly systemSpecs: ReadonlyMap<string, SystemSpec>;
  readonly systemTraceRows: readonly SystemTraceRow[];
  readonly systemTraceIssues: readonly string[];
  readonly sysReferences: ReadonlySet<string>;
  readonly riskProfileAdopted: boolean;
  readonly riskProfile: ReadonlyMap<string, RiskProfileEntry>;
  readonly riskProfileIssues: readonly string[];
}
...
function fields(section: string, names: readonly string[]): ReadonlyMap<string, string> {
...
function ids(value: string | undefined, pattern: RegExp): ReadonlySet<string> {
...
function declaredIds(content: string, pattern: RegExp): ReadonlySet<string> {
  const declarations = [
...
function records(content: string, pattern: RegExp): {
  ...
  for (const match of wanted) {
    ...
    else values.set(id, content.slice(start, end));
  }
  return { values, duplicates, invalid };
}

export function sectionPresent(content: string, heading: string): boolean {
...
export function artifactComplete(content: string, type: string, headings: readonly string[]): boolean {
  const frontmatter = /^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)/.exec(content)?.[1];
  if (!frontmatter || !new RegExp(`^type:\\s*${escaped(type)}\\s*$`, "mi").test(frontmatter) || !/^status:\s*(?:complete|amended)\s*$/mi.test(frontmatter)) return false;
  return headings.every((heading) => sectionPresent(content, heading));
}

export function writeSetPathIssue(value: string): PathIssue | undefined {
  if (/[*<>\\]|\.\.\./.test(value)) return { code: "writeset_glob", reason: "write-set paths must not contain wildcard or placeholder syntax" };
  if (
    !value
...
export function classifyWriteSetClause(writeSet: string): WriteSetClauseRejection | undefined {
  let inCode = false;
  let masked = "";
...
const NARRATIVE_FIELDS = ["Command", "Positive case", "Negative case", "Evidence", "Evidence target"] as const;
 ...
 */
function procedureNarratives(seitDocument: string): ReadonlyMap<string, readonly ProcedureNarrative[]> {
  ...
  const narratives = new Map<string, ProcedureNarrative[]>();
  for (let index = 0; index < headings.length; index += 1) {
    const section = headings[index];
    if (section.level !== 2 || !/procedur/i.test(section.title)) continue;
...
function sysIds(value: string): ReadonlySet<string> {
  if (!value) return new Set();
 ...
 */
function systemMap(designDocument: string): {
  readonly adopted: boolean;
  readonly catalog: ReadonlyMap<string, SystemCatalogEntry>;
  readonly catalogIssues: readonly string[];
  readonly specs: ReadonlyMap<string, SystemSpec>;
  readonly traceRows: readonly SystemTraceRow[];
  readonly traceIssues: readonly string[];
  ...
  if (!catalogTable.length) {
    ...
    if (headers.length !== SYSTEM_CATALOG_COLUMNS.length || SYSTEM_CATALOG_COLUMNS.some((column) => !headers.includes(column))) {
      ...
      const cell = (values: string[], header: string): string => values[headers.indexOf(header)] ?? "";
  ...
  const specs = new Map<string, SystemSpec>();
  ...
  const traceRows: SystemTraceRow[] = [];
 ...
 */
function riskProfile(planDocument: string): {
  readonly adopted: boolean;
  readonly flags: ReadonlyMap<string, RiskProfileEntry>;
  readonly issues: readonly string[];
} {
  const adopted = RISK_PROFILE_HEADING.test(planDocument);
  if (!adopted) return { adopted: false, flags: new Map(), issues: [] };
  ...
  const flags = new Map<string, RiskProfileEntry>();
  const issues: string[] = [];
  const cell = (values: string[], header: string): string => values[headers.indexOf(header)] ?? "";
  for (const line of table.slice(2)) {
    ...
    if (!KNOWN_RISK_FLAGS.includes(flag as KnownRiskFlag)) {
  ...
  for (const flag of KNOWN_RISK_FLAGS) {
    if (!flags.has(flag)) issues.push(`Risk Profile is missing the ${flag} flag`);
  }
  return { adopted, flags, issues };
}

function parseRiskCoverage(flag: string, body: string, issues: string[]): RiskProfileEntry {
  const clauses: RiskCoverageClause[] = [];
  ...
  for (const clause of body.split(";")) {
    ...
    const kind = match[1].toLowerCase() as RiskCoverageClauseKind;
    ...
    if (kind === "system") {
      for (const token of value.split(",").map((part) => part.trim()).filter(Boolean)) {
        if (!RISK_SYSTEM_TOKEN.test(token)) coverageIssues.push(`${flag}: system coverage must name a typed SYS- id: ${token}`);
        else clauses.push({ kind, value: token.toUpperCase() });
      }
    } else if (kind === "seit") {
      for (const token of value.split(",").map((part) => part.trim()).filter(Boolean)) {
        if (!SEIT_ID.test(token)) coverageIssues.push(`${flag}: SEIT coverage must name typed SEIT- row ids: ${token}`);
        else clauses.push({ kind, value: token.toUpperCase() });
      }
    } else if (kind === "slice") {
      for (const token of value.split(",").map((part) => part.trim()).filter(Boolean)) {
        if (!SLICE_ID_FORMAT.test(token)) coverageIssues.push(`${flag}: slice coverage must name typed slice ids: ${token}`);
        else clauses.push({ kind, value: token });
      }
    } else {
...
function traceability(seitDocument: string): {
  readonly headers: readonly string[];
  readonly rows: ReadonlyMap<string, TraceabilityRow>;
  readonly commands: ReadonlySet<string>;
  readonly issues: readonly { readonly observed: string; readonly reason: string }[];
  readonly identifierValues: readonly string[];
} {
  ...
  const issues: { observed: string; reason: string }[] = [];
...
function waveMap(implementationDocument: string): ReadonlyMap<number, ReadonlySet<string>> {
  const waves = new Map<number, Set<string>>();
  for (const line of implementationDocument.split(/\r?\n/)) {
    ...
    for (let index = 0; index < declarations.length; index += 1) {
      ...
      const members = waves.get(Number(declaration[1])) ?? new Set<string>();
...
function dependencyMap(implementationDocument: string): ReadonlyMap<string, ReadonlySet<string>> {
  ...
  for (const line of implementationDocument.split(/\r?\n/).filter((value) => /(?:-->|──>|→)/.test(value))) {
    ...
    if (!groups[0].length && branchSource) groups[0] = [branchSource];
    for (let index = 0; index + 1 < groups.length; index += 1) {
      for (const source of groups[index]) {
        const targets = result.get(source) ?? new Set<string>();
...
function sharedInterfaces(value: string): readonly string[] | undefined {
...
function validIntegrationBoundary(value: string): boolean {
...
function parallelSafe(value: string): boolean | undefined {
...
function parsePhaseGraph(implementationDocument: string): {
  readonly entries: readonly PlanPhaseGraphEntry[];
  readonly issues: readonly string[];
} {
  if (!/^##[ \t]+Phase graph[ \t]*$/mi.test(implementationDocument)) return { entries: [], issues: [] };
  const content = section(implementationDocument, "Phase graph");
  if (!content) return { entries: [], issues: ["Phase graph section is empty"] };
  const lines = content.split(/\r?\n/).filter((line) => line.trim().startsWith("|"));
  if (lines.length < 3) return { entries: [], issues: ["Phase graph must be a Markdown table"] };
  ...
  const entries: PlanPhaseGraphEntry[] = [];
  const issues: string[] = [];
  for (const line of lines.slice(2)) {
...
export function parsePlanDocuments(documents: PlanDocuments): PlanModel {
  ...
  const manifests = new Map<string, PlanManifest>();
...
function finding(
  ...
  remedy: string,
  sliceId?: string,
  severity: StructuralFinding["severity"] = "amendment",
): StructuralFinding {
  return { code, severity, artifact, ...(sliceId ? { sliceId } : {}), observed: observed.slice(0, 512), required, remedy };
}

function multilineField(sectionText: string, name: string): boolean {
...
function invalidIdentifiers(model: PlanModel): readonly string[] {
  ...
  return [...new Set(candidates.filter((candidate) => {
    if (/^(?:AC|RISK)-/i.test(candidate)) return !PLAN_ID.test(candidate);
    if (/^(?:DES|CONTRACT)-/i.test(candidate)) return !DESIGN_ID.test(candidate);
    if (/^SEIT-/i.test(candidate)) return !SEIT_ID.test(candidate);
    if (/^(?:CMD|PROC)-/i.test(candidate)) return !COMMAND_ID.test(candidate);
    return true;
  }))];
}

type InputKind = "metric denominator" | "ledger key" | "contract field";
...
function explicitInputReferences(sectionText: string, fieldName: "Inputs" | "Produces"): readonly InputReference[] {
  ...
  for (const match of value.matchAll(/\b(metric denominator|ledger key|contract field)\s+`([^`]+)`/gi)) {
    const kind = match[1].toLowerCase() as InputKind;
    const name = match[2];
    if (name.length > 128 || !/^[A-Za-z][A-Za-z0-9]*(?:\.[A-Za-z][A-Za-z0-9]*)*$/.test(name)) continue;
    const key = `${kind}\u0000${name}`;
...
function isRecordedLedgerKey(name: string): boolean {
  if (RECORDED_LEDGER_KEYS.has(name)) return true;
  const payloadKey = /^payload\.([A-Za-z][A-Za-z0-9]*)$/.exec(name)?.[1];
  return payloadKey !== undefined && RECORDED_LEDGER_KEYS.has(payloadKey);
}

const RISK_PROFILE_REQUIRED =
  "the Risk Profile must be a Markdown table with the Flag, Applies, and Coverage or rationale columns enumerating every known flag with applies yes or no, each yes flag mapping a design section or SYS- system, a SEIT row, and a slice, and each no flag carrying an evidence-backed rationale";

export function structuralFindings(model: PlanModel): readonly StructuralFinding[] {
  const findings: StructuralFinding[] = [];
  const push = (...value: Parameters<typeof finding>): void => { findings.push(finding(...value)); };
  ...
  for (const [artifact, content, type] of [
    ...
    if (!artifactComplete(content, type, [])) push("artifact_frontmatter_invalid", artifact, "invalid or incomplete frontmatter", `type: ${type} and status: complete or amended`, "repair the artifact frontmatter");
  }

  for (const heading of ["Required Commands", "Traceability Matrix", "Cross-cutting Checks"]) {
    if (!sectionPresent(model.documents.seit, heading)) push("seit_section_missing", "seit.md", heading, `non-empty ${heading} section`, `add the ${heading} section`);
  }
  for (const heading of ["Use Cases and Communication Flows", "Interface Option Check", "OOPDSA Implementation Design"]) {
    if (!sectionPresent(model.documents.design, heading)) push("design_section_missing", "design.md", heading, `non-empty ${heading} section`, `add the ${heading} section`);
  }

  for (const [id, slice] of model.slices) {
    for (const name of requiredSliceFields) {
      if (!slice.fields.get(name)) push("slice_field_missing", "implementation.md", name, `non-empty ${name} field`, `add ${name} to Slice ${id}`, id);
    }
    ...
    if (!slice.requirementIds.size) push("id_unknown", "implementation.md", slice.fields.get("Requirement IDs") ?? "", "Requirement IDs must contain at least one defined AC or RISK id", "add a defined requirement id", id);
    if (!slice.designIds.size) push("id_unknown", "implementation.md", slice.fields.get("Design IDs") ?? "", "Design IDs must contain at least one defined DES or CONTRACT id", "add a defined design id", id);
    if (!slice.proofRowIds.size) push("id_unknown", "implementation.md", slice.fields.get("SEIT proof rows") ?? "", "SEIT proof rows must contain at least one defined SEIT id", "add a defined proof row id", id);
  }
  ...
  // the names match.
  const producedKey = (kind: string, name: string): string => `${kind} ${name}`;
  const producedInputs = new Set(
    [...model.slices.values()].flatMap((slice) => explicitInputReferences(slice.raw, "Produces").map((reference) => producedKey(reference.kind, reference.name))),
  );
  for (const [id, slice] of model.slices) {
    for (const input of explicitInputReferences(slice.raw, "Inputs")) {
      if (producedInputs.has(producedKey(input.kind, input.name)) || isRecordedLedgerKey(input.name)) continue;
      push(
        "input_unproduced",
        "implementation.md",
        `${input.kind}: ${input.name}`,
        "every explicitly declared input must name a producer in the plan or an existing recorded ledger key",
        `declare a producer for ${input.name} or remove the unsupported input`,
        id,
      );
    }
  }
  ...
  const recognizedNoWritePlan = [...model.manifests.values()].some((manifest) =>
    /\b(?:none|no writes?(?: required)?|no (?:new|required|source|product) files?)\b/i.test(manifest.fields.get("Write set") ?? ""));
  if (model.manifests.size && !allPaths.size && !recognizedNoWritePlan) push("writeset_empty", "implementation.md", "plan declares no writable path", "the execution boundary requires at least one literal writable path", "assign at least one bounded write target");
  if (allPaths.size > MAX_ITEMS) push("writeset_unsafe_path", "implementation.md", `${allPaths.size} unique paths`, `write sets may aggregate at most ${MAX_ITEMS} paths`, "split the execution contract");
  if (allCommands.size > MAX_ITEMS) push("command_undeclared", "implementation.md", `${allCommands.size} unique commands`, `manifests may aggregate at most ${MAX_ITEMS} commands`, "reduce the evidence command set");

  for (const [id, manifest] of model.manifests) {
    for (const name of requiredManifestFields) {
      if (!manifest.fields.get(name)) push("manifest_field_missing", "implementation.md", name, `non-empty ${name} field`, `add ${name} to the ${id} execution manifest`, id);
    }
    const writeSet = manifest.fields.get("Write set") ?? "";
    const noWrites = /\b(?:none|no writes?(?: required)?)\b/i.test(writeSet);
    if (multilineField(manifest.raw, "Write set")) push("writeset_multiline", "implementation.md", writeSet, "Write set must fit on the field line", "move every write target onto the Write set line", id);
    if (!noWrites && manifest.writeSetPaths.length && !/\bonly\b/i.test(writeSet)) push("writeset_missing_only", "implementation.md", writeSet, "Write set must contain the literal word only", "state the bounded paths as write-only targets", id);
    if (noWrites || !manifest.writeSetPaths.length) push("writeset_empty", "implementation.md", writeSet, "each executable slice must declare one or more literal paths", "assign a bounded write target", id);
    const seen = new Set<string>();
    for (const path of [...writeSet.matchAll(/`([^`]+)`/g)].map((match) => match[1])) {
      const issue = writeSetPathIssue(path);
      if (issue) push(issue.code, "implementation.md", path, issue.reason, "replace it with a bounded normalized repository-relative literal", id);
      if (seen.has(path)) push("writeset_duplicate", "implementation.md", path, "each path may appear once per write set", "remove the duplicate path", id);
      seen.add(path);
    }
    if (manifest.writeSetClauseRejection) {
      push(
        "writeset_readonly_harvest",
        "implementation.md",
        manifest.writeSetClauseRejection.clause,
        manifest.writeSetClauseRejection.reason,
        "Restate the prohibition in prose without backticks, per planning contract rule 5.",
        id,
      );
    }
    for (const command of manifest.commandIds) {
      if (command === "CMD-BUILD") push("build_command_in_manifest", "implementation.md", command, "CMD-BUILD must run only after Focus validation", "remove CMD-BUILD from the slice manifest", id);
    }
  ...
  for (let left = 0; left < manifestEntries.length; left += 1) {
    for (let right = left + 1; right < manifestEntries.length; right += 1) {
      ...
      if (!leftManifest.parallelSafe && !rightManifest.parallelSafe) continue;
      ...
      if (!overlap.length && !declaredIndependenceConflict) continue;
      findings.push(finding(
        "parallel_safety_conflict",
        "implementation.md",
        overlap.length
          ? `${leftId}, ${rightId}: ${overlap.join(", ")}`
          : `${leftId}, ${rightId}: write sets are not provably disjoint`,
        "slices declaring Parallel safe yes must have provably disjoint write sets",
        "declare Parallel safe no or separate the write sets",
      ));
    }
  }
  ...
  if (model.phaseGraph.length) {
    ...
    if (phaseIds.size !== model.phaseGraph.length
      ...
      || model.phaseGraph.some((phase) =>
        phase.dependsOnPhases.some((dependency) => dependency === phase.phaseId || !phaseIds.has(dependency)))) {
  ...
  for (const invalid of invalidIdentifiers(model)) push("id_format_invalid", "implementation.md", invalid, "identifiers must match their closed prefix grammar", "replace the malformed identifier");
  ...
  for (const [id, slice] of model.slices) {
    for (const value of slice.requirementIds) if (!model.planIds.has(value)) push("id_unknown", "implementation.md", value, "slice requirement ids must be defined in plan-spec.md", "define or remove the identifier", id);
    for (const value of slice.designIds) if (!model.designIds.has(value)) push("id_unknown", "implementation.md", value, "slice design ids must be defined in design.md", "define or remove the identifier", id);
    for (const value of slice.proofRowIds) if (!model.traceRows.has(value)) push("id_unknown", "implementation.md", value, "slice SEIT ids must name a traceability row", "define or remove the identifier", id);
  }

  if (
    model.traceHeaders.length !== TRACE_HEADERS.length
  ...
  for (const line of table.slice(2)) {
    ...
    if (placeholder) push("trace_cell_placeholder", "seit.md", `${placeholder}: ${values[model.traceHeaders.indexOf(placeholder)] ?? ""}`, "every required traceability cell must contain evidence", "replace the placeholder");
  }

  for (const match of model.documents.implementation.matchAll(/\bSlice\s+((?:[A-Za-z]+\d+|\d+(?:\.\d+)+)(?![A-Za-z0-9_-]|\.\d))/g)) {
    if (!sliceIds.has(match[1])) push("slice_reference_dangling", "implementation.md", match[1], "every Slice reference must name a declared slice", "declare or remove the dangling reference");
  }
  ...
  for (const id of dependencyIds) {
    if (!sliceIds.has(id)) push("slice_reference_dangling", "implementation.md", id, "every dependency endpoint must name a declared slice", "declare or remove the dangling dependency");
  }
  ...
  for (const members of model.waves.values()) {
    for (const id of members) membership.set(id, (membership.get(id) ?? 0) + 1);
  }
  ...
  for (const [id, manifest] of model.manifests) {
    ...
    if (!manifest.commandIds.size) push("command_undeclared", "implementation.md", "no command ids", "each manifest needs a declared and traceable command", "add an evidence command", id);
    for (const command of manifest.commandIds) {
      if (!model.requiredCommands.has(command) || !mappedCommands.has(command)) push("command_undeclared", "implementation.md", command, "manifest commands must be declared and reachable through the slice proof rows", "declare and trace the command", id);
    }
  }
